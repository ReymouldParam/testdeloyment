    ...
    public function add() {
        ...
        if ($this->data) {
            ...
            if (isset($msgString) && $msgString != '') {
                ...
                $this->request->data['Category']['cat_id'] = 0;
                $this->request->data['Category']['parent_id'] = 0;
                $this->request->data['Category']['meta_keyphrase'] = '';
                $this->request->data['Category']['page_header'] = '';
                $this->request->data["Category"]["image"] ='';
                $this->request->data['Category']['meta_keywords'] = '';
                $this->request->data['Category']['meta_title'] = trim($this->data['Category']['meta_title']);
                $this->request->data['Category']['meta_description'] = trim($this->data['Category']['meta_description']);
                $this->request->data['Category']['slug'] = $this->stringToSlugUnique($this->data["Category"]["name"], 'Category');
                $this->request->data['Category']['status'] = '1';
                $this->request->data['Category']['keywords'] = trim($this->data['Category']['keywords']);
                $this->request->data['Category']['created_by'] =  $this->Session->read('user_id');
                if ($this->Category->save($this->data)) {
                    $lastInsertedId = $this->Category->getInsertID();
                    $msgString .="- Category Added Successfully.<br>";
                    echo json_encode(array('status' => 'success', 'message' =>$msgString, 'name'=> $this->data["Category"]["name"], 'id' => $lastInsertedId ));
                    exit;
                }
            }
        }
