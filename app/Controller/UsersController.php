...
//            }
    ...
    public function admin_deleteusers($slug = NULL, $type = NULL) {
        ...
        if ($slug != '') {
			...
			$this->loadModel('JobNotification');
			$this->loadModel('Alert');
			$this->loadModel('CoverLetter');
			$this->loadModel('ShortList');
			$this->loadModel('Professional');
			$this->loadModel('JobApplyCandidate');
			$this->Alert->deleteAll(array('Alert.user_id' => $userId));
			$this->AlertJob->deleteAll(array('AlertJob.user_id' => $userId));
			
			$certificateArray = $this->Certificate->find('list', array('conditions' => array('Certificate.user_id' => $userId), 'fields' => array('document')));
			if(!empty($certificateArray)){
				foreach ($certificateArray as $value) {
					if(!empty($value)){
						if (file_exists(UPLOAD_CERTIFICATE_PATH . $value)) {
			...
			$this->Certificate->deleteAll(array("Certificate.user_id" => $userId));
			$this->CoverLetter->deleteAll(array("CoverLetter.user_id" => $userId));
			$this->Education->deleteAll(array("Education.user_id" => $userId));
			$this->Experience->deleteAll(array("Experience.user_id" => $userId));
			$this->Favorite->deleteAll(array("Favorite.user_id" => $userId));
			$this->Favorite->deleteAll(array("Favorite.candidate_id" => $userId));
			$jobsList = $this->Job->find('all', array('conditions' => array('Job.user_id' => $userId)));
			if(!empty($jobsList)){
				foreach ($jobsList as $key => $val) {
					$this->AlertJob->deleteAll(array('AlertJob.job_id' => $val['Job']['id']));
					$this->JobApply->deleteAll(array('JobApply.job_id' => $val['Job']['id']));
					$this->JobNotification->deleteAll(array('JobNotification.job_id' => $val['Job']['id']));
					$this->Mail->deleteAll(array('Mail.job_id' => $val['Job']['id']));
					$this->ShortList->deleteAll(array('ShortList.job_id' => $val['Job']['id']));
					if(!empty($val['Job']['logo'])){
						if (file_exists(UPLOAD_JOB_LOGO_PATH . $val['Job']['logo'])) {
							@unlink(UPLOAD_JOB_LOGO_PATH . $val['Job']['logo']);
						}
						
					}
				}
			}
			$this->Job->deleteAll(array("Job.user_id" => $userId));
			$this->JobApply->deleteAll(array("JobApply.user_id" => $userId));
			$this->JobNotification->deleteAll(array("JobNotification.user_id" => $userId));
			$this->Mail->deleteAll(array("Mail.to_id" => $userId));
			$this->Mail->deleteAll(array("Mail.from_id" => $userId));
			$this->JobApplyCandidate->deleteAll(array('JobApplyCandidate.user_id' => $id));
			$this->Payment->deleteAll(array("Payment.user_id" => $userId));
			$this->Professional->deleteAll(array("Professional.user_id" => $userId));
			$this->ShortList->deleteAll(array("ShortList.user_id" => $userId));
			$this->UserPlan->deleteAll(array("UserPlan.user_id" => $userId));
			//end remove data
			if ($this->User->delete($userId)) {
				if (file_exists(UPLOAD_FULL_PROFILE_IMAGE_PATH . $image)) {
					@unlink(UPLOAD_FULL_PROFILE_IMAGE_PATH . $image);
				}
				if (file_exists(UPLOAD_THUMB_PROFILE_IMAGE_PATH . $image)) {
					@unlink(UPLOAD_THUMB_PROFILE_IMAGE_PATH . $image);
				}
				if (file_exists(UPLOAD_SMALL_PROFILE_IMAGE_PATH . $image)) {
    ...
    public function recreateAccount(){
        $this->autoRender = false;
        $email = $this->Session->read('deactivated_user');
        $this->Session->delete('deactivated_user');
        if(!empty($email)){
            $user_info = $this->User->find('first', array('conditions' => array('User.email_address' => $email)));
            $this->deleteAccountPermanantly($user_info['User']['id']);
    ...
    public function reActiveAccount(){
        ...
        if(!empty($email)){
            ...
            $this->User->updateAll(array('User.status' => "'1'", 'User.deactivate_date' => NULL), array("User.id" => $user_id));
            $this->Job->updateAll(array('Job.status' => "'1'"), array("Job.user_id" => $user_id));
    ...
    function gmaillogin() {
		...
		require_once '../Config/google_login.php';
		$client = new Google_Client();
		$client->setApplicationName('GriffeRack');
		$client->setClientId(CLIENT_ID);
		$client->setClientSecret(CLIENT_SECRET);
		$client->setRedirectUri(CLIENT_REDIRECT_URL);
		$client->setDeveloperKey(GMAIL_DEVELOPER_KEY);
		$client->setScopes(array('https://www.googleapis.com/auth/plus.login','https://www.googleapis.com/auth/userinfo.email', 'https://www.googleapis.com/auth/plus.me'));
		$client->setApprovalPrompt('auto');
		...
		if(isset($_REQUEST['code'])) {
			...
			$this->Session->write("google_access_token", $client->getAccessToken());
		}
		if ($this->Session->read('google_access_token')){ 
			$client->setAccessToken($this->Session->read('google_access_token'));
		}
		if ($client->getAccessToken()){
			$user_info = $oauth2->userinfo->get();
			$googleUserId = trim($user_info['id']);
			$firstName = trim(filter_var($user_info['given_name']));
			$lastName = trim(filter_var($user_info['family_name']));
			$name = trim(filter_var($user_info['name']),FILTER_SANITIZE_SPECIAL_CHARS);
			$email = trim(filter_var($user_info['email'], FILTER_SANITIZE_EMAIL));
			$profile_image_url = trim(filter_var($user_info['picture'], FILTER_VALIDATE_URL));
			$queryGoogle = $this->User->find("first", array("conditions" => array("User.google_user_id" => $googleUserId)));
			$msgString = "";
			
			if(!empty($queryGoogle)){
				if ($queryGoogle['User']['activation_status'] == 0) {
					$this->Session->delete('resend_array');
					$username = $name;
					$userId = $queryGoogle['User']['id'];
					$link = HTTP_PATH . "/users/confirmation/" . $userId . "/" . md5($userId) . "/" . urlencode($email);
					$resend_array = array('username' => $username, 'email' => $email, 'link' => $link, 'userType' => 'checked');
					$this->Session->write("resend_array", $resend_array);
					$msgString .= __d('controller', 'Please check your email for activation link to activate your account. If you have not received the activation link, click on the button below for us to resend the activation link.<a href="/users/resendEmail"><input class="btn_same btn btn-primary" type="button" value="Resend"></a>', true) . "<br>";
				} 
				else if ($queryGoogle['User']['status'] == 0) {
					
					$deactivated_account = $this->User->isDeactivatedAccount($queryGoogle['User']['email_address']);
					if(!empty($deactivated_account)){
                        $deactivateDate = strtotime($deactivated_account['User']['deactivate_date']);
                        $today = strtotime(date('Y-m-d'));
                        $thirtyDaysAgo = strtotime('-30 days', $today);
                        
                        if ($deactivateDate >= $thirtyDaysAgo && $deactivateDate <= $today) {
							$this->Session->write('deactivated_user', $queryGoogle['User']['email_address']);
							$this->redirect("/users/login");
                        } else {
                            $this->deleteAccountPermanantly($deactivated_account['User']['id']);
                        }
                    }
					else{
			...
			$queryGoogleEmail = $this->User->find("first", array("conditions" => array("User.email_address" => $email)));
			
			if(!empty($queryGoogleEmail)){
				if ($queryGoogleEmail['User']['activation_status'] == 0) {
					$this->Session->delete('resend_array');
					$username = $name;
					$userId = $queryGoogleEmail['User']['id'];
					$link = HTTP_PATH . "/users/confirmation/" . $userId . "/" . md5($userId) . "/" . urlencode($email);
					$resend_array = array('username' => $username, 'email' => $email, 'link' => $link, 'userType' => 'checked');
					$this->Session->write("resend_array", $resend_array);
					$msgString .= __d('controller', 'Please check your email for activation link to activate your account. If you have not received the activation link, click on the button below for us to resend the activation link.<a href="/users/resendEmail"><input class="btn_same btn btn-primary" type="button" value="Resend"></a>', true) . "<br>";
				...
				else if ($queryGoogleEmail['User']['status'] == 0) {
			...
			if (isset($msgString) && $msgString != '') {
               ...
               $this->redirect("/users/login");
			...
			else if(!empty($queryGoogle) && !empty($queryGoogleEmail)){ // login
				$this->Session->write("user_id", $queryGoogle['User']['id']);
				$this->Session->write("user_type", $queryGoogle['User']['user_type']);
				$this->Session->write("email_address", $queryGoogle['User']['email_address']);
				$this->Session->write("user_name", $queryGoogle['User']['first_name']);
				...
				// added default plan
					...
					$paymentInfo = $this->Plan->find('first', array('conditions'=>array('Plan.slug'=>'reyfer-welcome')));
					if(!empty($paymentInfo)){
						$userPlanInfo = $this->UserPlan->find('first', array('conditions'=>array('UserPlan.user_id'=>$userId)));
						if(empty($userPlanInfo)){
						
							$this->request->data['UserPlan']['payment_id']  = "";
							$this->request->data['UserPlan']['user_id']  = $userId;
							$this->request->data['UserPlan']['plan_id']  = $paymentInfo['Plan']['id'];
							...
							$this->request->data['UserPlan']['slug']  = 'uplan-'.$userId.time();
							$this->request->data['UserPlan']['invoice_no'] = $max_invoice_no;
							$this->UserPlan->save($this->request->data['UserPlan']);
						}
					}
					
					// end plan
				if(empty($queryGoogle['User']['contact'])){
					$this->redirect("/users/basicProfile");
				}
				else
				{
					$this->redirect("/jobs/listing");
				}
			}
			else if(empty($queryGoogle) && !empty($queryGoogleEmail)){ // login
				
				$this->User->updateAll(array('User.google_user_id' =>$googleUserId), array('User.id' => $queryGoogleEmail['User']['id']));
				$this->Session->write("user_id", $queryGoogleEmail['User']['id']);
				$this->Session->write("user_type", $queryGoogleEmail['User']['user_type']);
				$this->Session->write("email_address", $queryGoogleEmail['User']['email_address']);
				$this->Session->write("user_name", $queryGoogleEmail['User']['first_name']);
				$this->Session->delete('Userloginstatus');
				// added default plan
				$userId = $queryGoogleEmail['User']['id'];
				$invoice_no = $this->UserPlan->find('first', array(
				...
				// end plan
				if(empty($queryGoogleEmail['User']['contact'])){
					$this->redirect("/users/basicProfile");
				}
				else
				{
				...
				$this->User->updateAll(array('User.google_user_id' =>$googleUserId), array('User.id' => $queryGoogle['User']['id']));
				$this->Session->write("user_id", $queryGoogle['User']['id']);
				$this->Session->write("user_type", $queryGoogle['User']['user_type']);
				$this->Session->write("email_address", $queryGoogle['User']['email_address']);
				$this->Session->write("user_name", $queryGoogle['User']['first_name']);
			...
			else if(empty($queryGoogle) && empty($queryGoogleEmail)){ //register
				$source = "Gmail";
				...
				$this->request->data['User']['google_user_id'] = $googleUserId;
				...
				if ($this->User->save($this->data)) {
					...
					$this->Session->write("user_name", $firstName);
					$this->Session->delete('Userloginstatus');
					$invoice_no = $this->UserPlan->find('first', array(
					'fields' => array('MAX(UserPlan.invoice_no) AS max_invoice_no')));
					$max_invoice_no = isset($invoice_no[0]['max_invoice_no']) ? $invoice_no[0]['max_invoice_no']+1 : '1';
					...
					$this->Session->write('success_msg', __d('controller', 'Your account is successfully created. Thank you!', true));
					$this->redirect("/users/basicProfile");
				}
...
//                </script>";
    ...
    public function deleteAccountPermanantly($userId) {
        $userCheck = $this->User->find('first', array('conditions' => array('User.id' => $userId)));
	...
	public function signup() {
		...
		if ($this->data) {
                ...
                if(isset($this->data["re_create_account"]) && !empty($this->data["re_create_account"])){
                    $this->deleteAccountPermanantly($this->data["re_create_account"]);
                }

                if(isset($this->data["re_activate_account"]) && !empty($this->data["re_activate_account"])){
                    $user_id = $this->data["re_activate_account"];
                    
                    $this->User->updateAll(array('User.status' => "'1'", 'User.deactivate_date' => NULL), array("User.id" => $user_id));
                    $this->Job->updateAll(array('Job.status' => "'1'"), array("Job.user_id" => $user_id));
                    $activated_user = $this->User->find('first', array('conditions' => "User.id = '" . $user_id . "'"));
                    
                    $this->Session->write('success_msg', __d('controller', 'Your account is successfully Activated.', true));

                    if ($activated_user['User']['user_type'] == 'recruiter') {
                ...
                if ($this->User->isRecordUniqueemail($this->data["User"]["email_address"]) == false) {
                    $deactivated_account = $this->User->isDeactivatedAccount($this->data["User"]["email_address"]);
                    ...
                    if(!empty($deactivated_account)){
                        ...
                        if ($deactivateDate >= $thirtyDaysAgo && $deactivateDate <= $today) {
                            $signup_again = true;
                            $this->set("deactivated_account", $deactivated_account['User']['id']);
                        } else {
                            $this->deleteAccountPermanantly($deactivated_account['User']['id']);
                        }
                    }else{
				...
				if (trim($this->data["User"]["contact"]) == '') {
                    $msgString .= __d('controller', 'Phone number is required field.', true) . "<br>";
                }
                if(!$signup_again){
                    if ($this->User->isRecordUniquecontact($this->data["User"]["contact"]) == false) {
                        $msgString .= __d('controller', 'Phone Number Already Exists.', true) . "<br>";
                    }
                }
                ...
                if ((isset($msgString) && $msgString != '') || ($signup_again == true)) {
                    $this->Session->write('error_msg', $msgString);
				...
				else 
				{
					...
					$this->request->data['User']['contact'] = trim($this->data['User']['contact']);
                    $this->request->data['User']['country_id'] = 1;
                    ...
                    if ($this->User->save($this->data)) {
                        ...
                        if ($this->data['User']['user_type'] == 'recruiter') {
							...
							if (!empty($adminDetail)) {
                               ...
                               // $this->Email->from = $site_title . "<" . $mail_from . ">";
                                ...
                                $this->Email->send();
                                $this->Email->reset();*/
								$toEmail = base64_encode($$adminDetail['Admin']['email']);
								$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
								$from = base64_encode($site_title . "<" . $mail_from . ">");
								$layout = 'default';
								$messageToSend = base64_encode($messageToSend);
						...
						else 
						{
                            if (!empty($this->Session->read('guest_user_cv'))) {
                                ...
                                $this->request->data['Certificate']['is_cv'] = 1;
						...
						$this->Session->delete('resend_array');
						$username = $userDetail["User"]["first_name"];
						...
						$this->Session->write('success_msg', __d('controller', 'Your account is successfully created, Please check your email for activation link. If you have not received the activation link, click on the button below for us to resend the activation link.<a href="/users/resendEmail"><input class="btn_same btn btn-primary" type="button" value="Resend"></a>', true));
...
//        }
    ...
    function login() {
        ...
        $this->set("returnurl", "");
        $this->userLoggedinCheck();
		$time = time();$time1 = date('Y-m-d H:i:s', $time);$authUrl = '';
        $type = 'jobseeker';
        $this->set('userType', $type);
        $this->gmailSession($type);
		$this->Session->write("type", $type);
		if (isset($this->data) && !empty($this->data)) {
            if (isset($_GET['job_id'])) {
                $returnUrl = $this->Session->read("returnUrl");
                $this->Session->write("returnUrl", $returnUrl.'?job_id='. $_GET['job_id']);
            }
			if (empty($this->data['User']['email_address'])) {
                $msgString .= "- Please enter your email address.<br>";
            } 
			...
			else 
			{
				...
				else 
				{
                    ...
                    if ($i == 1) {
                        //$this->Session->write('error_msg', 'Invalid email and/or password . you have five more attempts.');
                        $this->Session->write('error_msg', __d('controller', 'Invalid email/password or you have not registered. you have five more attempts.', true));
                    }
                    if ($i == 2) {
                        $this->Session->write('error_msg', __d('controller', 'Invalid email/password or you have not registered. you have four more attempts.', true));
                    }
                    if ($i == 3) {
                        $userCheck = $this->User->find("first", array("conditions" => array("User.email_address" => $email)));
                        if (!empty($userCheck)) {
                        ...
                        $this->Session->write('error_msg', __d('controller', 'Invalid email/password or you have not registered. you have three more attempts.', true));
                    }
                    if ($i == 4) {
                        $this->Session->write('error_msg', __d('controller', 'Invalid email/password or you have not registered. you have two more attempts.', true));
                    }
                    if ($i == 5) {
                        $this->Session->write('error_msg', __d('controller', 'Invalid email/password or you have not registered. you have one more attempts.', true));
                    }
                    if ($i == 6) {
                        $userCheck = $this->User->find("first", array("conditions" => array("User.email_address" => $email)));
                        if ($userCheck['User']['modified'] <= date('Y-m-d H:i:s', time())) {
                        ...
                        $this->Session->write('error_msg', __d('controller', 'Invalid email/password or you have not registered.', true));
                    }
                    $this->request->data['User']['captcha'] = '';
                }
            }
...
//                        </script>";
    ...
    public function myaccount() {
       ...
       //$this->recruiterAccess();
		...
		$showOldCVImages = $this->Certificate->find('all', array('conditions' => array('Certificate.user_id' => $userId, 'type' => 'image','is_cv' => 1)));
        $this->set('showOldCVImages', $showOldCVImages);
        $showOldCVDocs = $this->Certificate->find('all', array('conditions' => array('Certificate.user_id' => $userId, 'type' => 'doc', 'is_cv' => 1)));
        $this->set('showOldCVDocs', $showOldCVDocs);

		$showOldImages = $this->Certificate->find('all', array('conditions' => array('Certificate.user_id' => $userId, 'type' => 'image','is_cv' => 0)));
        $this->set('showOldImages', $showOldImages);
        $showOldDocs = $this->Certificate->find('all', array('conditions' => array('Certificate.user_id' => $userId, 'type' => 'doc', 'is_cv' => 0)));
        ...
        $designationList = $this->Skill->find('list', array('conditions' => array('Skill.type' => 'Designation', 'Skill.status' => '1'), 'fields' => array('Skill.name', 'Skill.name'), 'order' => 'Skill.name asc'));
		...
		if(!empty($this->data['User'])) {
            ...
            if (!empty($this->data["User"]["profile_image"]["name"])) {
                ...
                if ($this->data['User']['profile_image']['size'] > '2097152') {
                    ...
                    $msgString .= __d('controller', 'Not Valid Image Extention.', true) . "<br>";
                }
            }
			if (empty($this->data["User"]["first_name"])) {
                $msgString .= __d('controller', 'First name is required field.', true) . "<br>";
			...
			else 
			{
                ...
                elseif(isset($this->data['User']['old_image']) && !empty($this->data['User']['old_image'])){
                ...
                if($this->request->data['User']['total_exp'] == '0'){
                    $this->Experience->deleteAll(array("Experience.user_id " => $this->Session->read("user_id")));
                }
		...
		else if(!empty($this->data['Education'])) {
		...
		else if(!empty($this->data['Experience'])) {
			// find previos experiance id
			$previosCurrentDetails = $this->Experience->find('first', array('conditions' => array('Experience.user_id' =>$this->Session->read("user_id"),'Experience.current_company' =>1)));
			...
			else 
			{
                ...
                if ($this->data['Experience']) {
                    foreach ($this->data['Experience'] as $experience) {
						...
						$this->request->data['Experience']['current_company'] = $experience['current_company'];
						if($experience['current_company']==1){
							$this->request->data['Experience']['to_year'] = date('Y');
						}
						else
						{
							$this->request->data['Experience']['current_company'] = 0;
                        ...
                        $this->request->data['Experience']['created'] = date("Y-m-d");
						...
						if (!empty($this->data['Experience']['company_name']) || !empty($this->data['Experience']['role']) || !empty($this->data['Experience']['designation'])) {
							...
							$nextCurrentDetails = $this->Experience->find('first', array('conditions' => array('Experience.user_id' =>$this->Session->read("user_id"),'Experience.current_company' =>1)));
							$changedCurrentComanyStatus = 0;
							if(!empty($previosCurrentDetails)){
								$previousCurrentCompanyId = $previosCurrentDetails['Experience']['id'];
								if(!empty($nextCurrentDetails)){
									$nextCurrentCompanyId = $nextCurrentDetails['Experience']['id'];
									if($previousCurrentCompanyId!=$nextCurrentCompanyId){
										$changedCurrentComanyStatus = 1;
									}
								}
							}
							else
							...
							if($changedCurrentComanyStatus==1){
								$this->User->id = $this->Session->read("user_id");
								$this->request->data['User']['email_otp'] = "";
								$this->request->data['User']['emp_verification_status'] = "Pending";
								$this->request->data['User']['verification_source'] = "";
								$this->request->data['User']['verification_date'] = "";
		...
		else 
		{
			...
			// education
			$educationDetails = $this->Education->find('all', array('conditions' => array('Education.user_id' => $userId),'order' =>array('Education.basic_year Desc','Education.id Desc')));
			$this->set('educationDetails',$educationDetails);
			$this->set('eduDetails',$educationDetails);
			if ($educationDetails) {
                foreach ($educationDetails as $eduDetail) {
			...
			//end education
			// experiance
			$expDetails = $this->Experience->find('all', array('conditions' => array('Experience.user_id' => $this->User->id),'order' =>array('Experience.from_year Desc','Experience.current_company Desc')));
            
            $this->set('expDetails', $expDetails);
			 $this->set('experienceDetails', $expDetails);
            if ($expDetails) {
                foreach ($expDetails as $expDetail) {
					...
					$expDetail['Experience']['current_company'] = $expDetail['Experience']['current_company'];
	...
	public function basicProfile() {
		...
		$showOldCertificateImages = $this->Certificate->find('all', array('conditions' => array('Certificate.user_id' => $userId, 'type' => 'image','is_cv' => 0)));
		$this->set('showOldCertificateImages', $showOldCertificateImages);
		...
		$this->set('showOldDocs', $showOldDocs);
		if ($this->data) {
            if (!empty($this->data["User"]["total_exp"] && $this->data["User"]["total_exp"] > 0 )) {
                if (empty($this->data["User"]["company_name"])) {
                    $msgString .= __d('controller', 'Company name is required field.', true) . "<br>";
                } 
                else 
			...
			if(!empty($this->data["User"]["contact"])){
				if ($this->User->isRecordUniquecontactEdit($this->data["User"]["contact"],$userId) == false) {
					$msgString .= __d('controller', 'Phone Number Already Exists.', true) . "<br>";
				}
			}
			if(empty($showOldCVImages) && empty($showOldCVDocs)){
			...
			{
                $this->User->id = $this->Session->read("user_id");
				if(!empty($this->data["User"]["skills"])){
					if ($this->data['User']['skills']) {
						$this->request->data['User']['skills'] = implode(',', $this->data['User']['skills']);
					}
				}
...
//            $this->set('cityList', $cityList);
    ...
    public function logout() {
		...
		$this->Session->delete('employeeVerficationLater');
		$this->Session->delete('employeeVerficationLaterFirst');
...
//                    die;
	...
	public function uploadresume() {
        ...
        $tagline = $this->getSiteConstant('tagline');
        $title_for_pages = $site_title . " :: " . $tagline . " - ";
        $this->set('title_for_layout', $title_for_pages . 'Upload Resume');

        $this->userLoginCheck();
        
        $userId = $this->Session->read("user_id");
        $msgString= '';
        if ($this->data['User']['filedata']) {
            $getextention = $this->PImage->getExtension($this->data['User']['filedata']['name']);
            $extention = strtolower($getextention);
            global $extentions_doc;
            // print_r($extentions_doc);exit;
            if ($this->data['User']['filedata']['size'] > '20971520') {
                $msgString .= "- Max file size upload is 20MB.<br>";
		...
		else
		{
			...
			$specialCharacters = array('#', '$', '%', '@', '+', '=', '\\', '/', '"', ' ', "'", ':', '~', '`', '!', '^', '*', '(', ')', '|', "'");
            $toReplace = "-";
            $this->request->data['User']['filedata']['name'] = str_replace($specialCharacters, $toReplace, $this->data['User']['filedata']['name']);
            $this->request->data['User']['filedata']['name'] = str_replace("&", "and", $this->data['User']['filedata']['name']);
            if ($this->data['User']['filedata']) {
                $cvArray = $this->data['User']['filedata'];
				//$cvArray['name'] = 'doc-' . $userId . time() . rand(111, 99999).'.'.$extention;
				...
				else 
				{
					...
					chmod(UPLOAD_CERTIFICATE_PATH .$returnedUploadCVArray[0], 0755);
					//$slug = 'doc-' . $userId . time() . rand(111, 99999);
					$this->request->data['Certificate']['document'] = $image;
					$this->request->data['Certificate']['type'] = 'doc';
                    $this->request->data['Certificate']['is_cv'] = 1;
					$this->request->data['Certificate']['slug'] = $this->stringToSlugUnique($this->data['User']['filedata'], 'Certificate', 'slug');
				...
				{
                    $this->Session->write('success_msg', __d('controller', 'Resume has been Uploaded successfully.', true));
                    $this->redirect('/users/uploadresume');
                }
				
            }
        }
		
        $this->set('uploadresume', 'active');
    }
...
//            }
    ...
    public function resendEmail() {
        ...
        $site_title = $this->getSiteConstant('title');
        $site_url = $this->getSiteConstant('url');
		if ($this->Session->check('resend_array')) {
			...
			if ($resend_array) {
				...
				$emailtemplateMessage = $this->Emailtemplate->find("first", array("conditions" => "Emailtemplate.id='53'"));
				$toSubArray = array('[!username!]', '[!HTTP_PATH!]', '[!SITE_TITLE!]', '[!activelink!]');
				$fromSubArray = array($username, HTTP_PATH, $site_title, $link);
				$subjectToSend = str_replace($toSubArray, $fromSubArray, $emailtemplateMessage['Emailtemplate']['subject']);
				$toRepArray = array('[!username!]', '[!HTTP_PATH!]', '[!SITE_TITLE!]', '[!activelink!]');
				...
				$cronUrl = base64_encode(BACKGROUND_PATH);
				exec("php -f  $path  $toEmail $subjectToSend $replyTo $from $layout $messageToSend $cronUrl $pdfAttach >/dev/null 2>&1 &",$exeoutput);
				$this->Session->write('success_msg', __d('controller', 'We have you an activation link again, please check your email for activation link', true));
				$this->redirect('/users/login');
    ...
    function linkedinlogin() {
		...
		require_once '../Config/linked_login.php';
		$adapter = new Hybridauth\Provider\LinkedIn( $linkconfig );
		try {
			$adapter->authenticate();
			$user = $adapter->getUserProfile();
			file_put_contents("e.txt",json_encode($user));
			
			$linkedInId = trim($user->identifier);
			$firstName = $user->firstName;
			$lastName = $user->lastName;
			$firstName = trim(filter_var($user->firstName));
			$lastName = trim(filter_var($user->lastName));
			$email = trim(filter_var($user->email, FILTER_SANITIZE_EMAIL));
			$profile_image_url = trim(filter_var($user->photoURL, FILTER_VALIDATE_URL));
			$queryGoogle = $this->User->find("first", array("conditions" => array("User.linkedin_id" => $linkedInId)));
			$msgString = "";
			file_put_contents("d2.txt",json_encode($queryGoogle));
			if(!empty($queryGoogle)){
				if ($queryGoogle['User']['activation_status'] == 0) {
					$this->Session->delete('resend_array');
					$username = $firstName;
			...
			if (isset($msgString) && $msgString != '') {
				file_put_contents("d3.txt",$msgString);
                $this->Session->write('error_msg', $msgString);
				$this->redirect("/users/login");
            }
			else if(!empty($queryGoogle) && !empty($queryGoogleEmail)){ // login
				...
				$this->User->updateAll(array('User.linkedin_id'=>"'$linkedInId'"), array('User.id' => $queryGoogleEmail['User']['id']));
				...
				$this->request->data['User']['first_name'] = $firstName;
				$this->request->data['User']['last_name'] = $lastName;
				$this->request->data['User']['linkedin_id'] = $linkedInId;
				if ($profile_image_url) {
					$chars = "abcdefghijkmnopqrstuvwxyz023456789";
					srand((double) microtime() * 1000000);
					$i = 1;
...
//                 pr($this->Email->send());
                //$this->Email->reset();
				$toEmail = base64_encode(trim($emailaddress));
				$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
				$from = base64_encode($site_title . "<" . $mail_from . ">");
				$layout = 'default';
				$messageToSend = base64_encode($messageToSend);
...
//                        $this->Email->send();
    ...
    function generateformat() {
        $this->set('default', '1');
        $this->layout = false;
        $filename = 'Import Jobs Template.xlsx';
