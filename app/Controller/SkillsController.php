...
class SkillsController extends AppController {
    ...
    public function add() {
        ...
        if ($this->data) {
            ...
            if (empty($this->data["Skill"]["name"])) {
                $msgString .="- $type Name is required field.<br>";
                echo json_encode(array('status' => 'error', 'message' =>$msgString));
                exit;
            }
            else if (empty($this->data["Skill"]["type"])) {
                $msgString .="- $type Type is required field.<br>";
                echo json_encode(array('status' => 'error', 'message' =>$msgString));
                exit;
            } else {
                $sw = explode(",", trim($this->data['Skill']['name']));
                ...
                foreach ($sw as $k => $v) {
                    ...
                    $fin = $this->Skill->find('all', array('conditions' => array('Skill.type' => "$type")));
                    //  pr($fin); exit;
                    if (isset($fin) && count($fin) > 0 && is_array($fin)) {
                        if ($this->Skill->in_array_r(trim($v), $fin)) {
                            $msgString .="- $type Already Exists.<br>";
                            echo json_encode(array('status' => 'error', 'message' =>$msgString));
                            exit;
                        } else {
                            if (!empty($v)) {
                                $s['Skill']['slug'] = $this->stringToSlugUnique($v, 'Skill', 'slug');
                                $s['Skill']['name'] = $v;
                                $s['Skill']['type'] = $this->data["Skill"]["type"];
                                $s['Skill']['created_by'] =  $this->Session->read('user_id');
                                $this->Skill->saveall($s);
                            }
                        }
                    } else {
                ...
                $msgString .="- $type Added Successfully.<br>";
                $lastInsertedId = $this->Skill->getInsertID();
                echo json_encode(array('status' => 'success', 'message' =>$msgString, 'name'=> $v, 'id' => $lastInsertedId  ));
                exit;
            }
        }
    }
