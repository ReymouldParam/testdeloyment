...
//                        pr($this->data);
    ...
    public function removeDeletedAccounts() {
        $this->layout = "";
        $thresholdDate = date('Y-m-d', strtotime('-30 days'));
        $deleted_users = $this->User->find("list", array(
            "conditions" => array(
                "User.status = 0",
                "User.deactivate_date IS NOT NULL",
                "User.deactivate_date < '{$thresholdDate}'"
            ),
        ...
        if(!empty($deleted_users)){
            ...
            foreach($deleted_users as $userId){
                $userCheck = $this->User->find('first', array('conditions' => array('User.id' => $userId),"fields" => array("User.profile_image")));
            ...
            $this->Alert->deleteAll(array('Alert.user_id IN' => $deleted_users));
            $this->AlertJob->deleteAll(array('AlertJob.user_id IN' => $deleted_users));
            $this->Certificate->deleteAll(array("Certificate.user_id IN" => $deleted_users));
            $this->CoverLetter->deleteAll(array("CoverLetter.user_id IN" => $deleted_users));
            $this->Education->deleteAll(array("Education.user_id IN" => $deleted_users));
            $this->Experience->deleteAll(array("Experience.user_id IN" => $deleted_users));
            $this->Favorite->deleteAll(array("Favorite.user_id IN" => $deleted_users));
            $this->Favorite->deleteAll(array("Favorite.candidate_id IN" => $deleted_users));
            $this->Job->deleteAll(array("Job.user_id IN" => $deleted_users));
            $this->JobApply->deleteAll(array("JobApply.user_id IN" => $deleted_users));
            $this->JobNotification->deleteAll(array("JobNotification.user_id IN" => $deleted_users));
            $this->Mail->deleteAll(array("Mail.to_id IN" => $deleted_users));
            $this->Mail->deleteAll(array("Mail.from_id IN" => $deleted_users));
            $this->Payment->deleteAll(array("Payment.user_id IN" => $deleted_users));
            $this->Professional->deleteAll(array("Professional.user_id IN" => $deleted_users));
            $this->ShortList->deleteAll(array("ShortList.user_id IN" => $deleted_users));
            $this->UserPlan->deleteAll(array("UserPlan.user_id IN" => $deleted_users));
