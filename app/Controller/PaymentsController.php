...
class PaymentsController extends AppController {
    ...
    public $uses = array('Payment', 'Admin', 'Job', 'Emailtemplate', 'User', 'JobNotification', 'UserPlan','JobCredit');
	...
	public function cronEmail() {
		...
		file_put_contents("dinesh.txt",base64_decode($this->data['pdfAttach']));
		...
		$this->Email->template = $this->data['template'];
...
//            $amountPaid = 100;
    ...
    public function success_new($buCode = null, $md5buCode = null) {
        ...
        $this->Email->send();*/
		$subjectToSend = "You have successfully purchase business account membership  on " . $site_title;
		$toEmail = base64_encode($email);
		$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
		$from = base64_encode($site_title . "<" . $mail_from . ">");
		$layout = 'default';
		$template = 'business_membership_renew';
		$messageToSend = "";
		$subjectToSend = base64_encode($subjectToSend);
		$path = WWW_ROOT.'cronEmail_template.php';
		$pdfAttach ="";
		$cronUrl = base64_encode(BACKGROUND_TEMPLATES_PATH);
		exec("php -f  $path  $toEmail $subjectToSend $replyTo $from $layout $template $messageToSend $cronUrl $pdfAttach >/dev/null 2>&1 &",$exeoutput);
...
//            $amountPaid = 100;
	...
	public function creditHistory() {
        ...
        $this->layout = "client";
        $this->set('creditHistory', 'active');

        $site_title = $this->getSiteConstant('title');
        $tagline = $this->getSiteConstant('tagline');
        $title_for_pages = $site_title . " :: " . $tagline . " - ";
        $this->set('title_for_layout', $title_for_pages . __d('controller', 'Credit History', true));
        $this->userLoginCheck();
        //$this->recruiterAccess();
		
        $userId = $this->Session->read("user_id");
		$userCredit = $this->User->field('credit', array('User.id' => $userId));
        $condition = array('JobCredit.user_id' => $userId);
        $separator = array();
        $urlSeparator = array();

        $order = 'JobCredit.id Desc';
        ...
        $this->set('separator', $separator);
        $this->set('urlSeparator', $urlSeparator);
        $this->paginate['JobCredit'] = array('conditions' => $condition, 'limit' => '20', 'page' => '1', 'order' => $order);
        $this->set('credits', $this->paginate('JobCredit'));
		$this->set('userCredit', $userCredit);
        if ($this->request->is('ajax')) {
            $this->layout = '';
            $this->viewPath = 'Elements' . DS . 'payments';
            $this->render('credit_history');
        }
    }
