...
class CandidatesController extends AppController {
    ...
    public $uses = array('Admin', 'Experience', 'Emailtemplate', 'User', 'Certificate', 'Country', 'State', 'City', 'Favorite', 'Job', 'JobApply', 'CoverLetter', 'Swear', 'PostCode', 'Course', 'Specialization', 'Education', 'Location', 'Skill', 'Plan', 'Download', 'Designation', 'ShortList', 'Alert', 'Pages', 'Setting', 'Professional', 'Mail', 'Payment', 'UserPlan');
        ...
        if ($this->data) {
            ...
            if (isset($msgString) && $msgString != '') {
                ...
                if (isset($msgString) && $msgString != '') {
                    ...
                    $this->request->data['User']['first_name'] = trim($this->data['User']['first_name']);
                    $this->request->data['User']['last_name'] = trim($this->data['User']['last_name']);
					if(!empty($this->request->data['User']['verification_date'])){
						if(!empty($this->request->data['User']['emp_verification_status'])){
							if($this->request->data['User']['emp_verification_status']=='Verified'){
								$verficationDate = date('Y-m-d',strtotime($this->request->data['User']['verification_date']));
								$this->request->data['User']['verification_date'] =$verficationDate; 
							}
						}
					}
                    if ($this->User->save($this->data)) {
    ...
    public function admin_verifyuser($slug = NULL) {
        $this->layout = "";
        if ($slug != '') {
            $id = $this->User->field('id', array('User.slug' => $slug));
            $cnd = array("User.id = $id");
            $currentDate = date('Y-m-d');
            $this->User->updateAll(array('User.emp_verification_status' => "'Verified'", 'User.verification_date' => "'". $currentDate."'"), $cnd);
            $user = $this->User->find('first', array('conditions' => array('User.id' => $id)));
        
            $this->set('user', $user);
            $this->viewPath = 'Elements' . DS . 'admin';
            $this->render('update_jobseeker');
        }
    }
...
//                die;
    ...
    public function admin_downloadVerificationDocument($filename = null) {
        set_time_limit(0);
        $file_path = UPLOAD_EMPLOYEE_VERIFICATION_PATH . $filename;
		$filename = substr($filename, 6);
        $this->Common->output_file($file_path, $filename);
        exit;
    }
	
	public function downloadDocCertificateProfile($filename = null) {
        set_time_limit(0);
        $file_path = UPLOAD_APPLY_JOB_CERTIFICATE_PATH . $filename;
        $filename = substr($filename, 6);
        $this->Common->output_file($file_path, $filename);
        exit;
    }
    ...
    public function uploadmultipleimages() {
        ...
        $this->layout = '';
        if ($this->data['Certificate']['cv']['name'] != '') {
            $is_cv = 1;
            $this->request->data['Certificate']['document'] = $this->data['Certificate']['cv'];
        }
        ...
        if (isset($msgString) && $msgString != '') {
            ...
            if (in_array($extention, $extentions)) {
                ...
                $returnedUploadImageArray = $this->PImage->upload($imageArray, UPLOAD_CERTIFICATE_PATH);
                //chmod(UPLOAD_CERTIFICATE_PATH .$returnedUploadImageArray[0], 0755);
				
				
                //$imageArray = $this->data['Certificate']['document'];
                //$imageArray['name'] = 'image-' . $userId . time() . rand(111, 99999).'.'.$extention;
				...
				else 
				{
                    ...
                    chmod(UPLOAD_CERTIFICATE_PATH .$returnedUploadImageArray[0], 0755);
					$slug = 'image-' . $userId . time() . rand(111, 99999);
					$this->request->data['Certificate']['type'] = 'image';
                    $this->request->data['Certificate']['slug'] = $slug;
					$this->request->data['Certificate']['is_cv'] = $is_cv;
        ...
        if ($msgString) {
            ...
            if (in_array($extention, $extentions)) {
                echo json_encode(array('status' => 'success', 'message' => '', 'image' => $image, 'type' => 'image','slug'=>$slug));
            } else {
                echo json_encode(array('status' => 'success', 'message' => '', 'image' => $image, 'type' => 'doc','slug'=>$slug));
            }
        }
        exit;
    }
...
//        $jsonStr = '{"email":"vikas.sharma@logicspice.com","username":"4","message":"test","subject":"subject1"}';
        ...
        $this->Email->send();
        $this->Email->reset();*/
		$toEmail = base64_encode($contact_details['Setting']['email']);
		$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
		$from = base64_encode($site_title . "<" . $mail_from . ">");
		$layout = 'default';
		$messageToSend = base64_encode($messageToSend);
...
//                    $this->Mail->save($maildata);
    ...
    public function sendmailemployer($slug = null) {
		...
		$mailToId = $mailDetail['Mail']['to_id'];
		$mailFromId = $mailDetail['Mail']['from_id'];
		$mailJobId = $mailDetail['Mail']['job_id'];
        if ($this->data) {
            if (empty($this->data["Candidate"]["id"])) {
                $msgString .= "- User Name is required field.<br>";
            }
            ...
            if (isset($msgString) && $msgString != '') {
                ...
                if ($userInfo) {
                    ...
                    $maildata['Mail']['to_id'] = $userInfo["User"]["id"];
                    $maildata['Mail']['from_id'] = $userId;
					$maildata['Mail']['job_id'] = $mailJobId;
                    $maildata['Mail']['slug'] = $this->stringToSlugUnique($this->data["Candidate"]["subject"], 'Mail', 'slug');
                    ;
                    $maildata['Mail']['subject'] = $this->data["Candidate"]["subject"];
                    $maildata['Mail']['message'] = $this->data["Candidate"]["message"];
...
//                    print_r($this->Email->send());
                ...
                //Disabled user with corresponding jobs
                $this->Job->updateAll(array('Job.status' => "'0'"), array("Job.user_id" => $userId));
                $this->User->updateAll(array('User.status' => "'0'", 'User.deactivate_date' => "'".date("Y-m-d")."'"), array("User.id" => $userId));
