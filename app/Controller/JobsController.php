...
class JobsController extends AppController {
    ...
    public $uses = array('Job', 'Admin', 'Experience', 'Emailtemplate', 'User', 'Certificate', 'Country', 'State', 'City', 'PromoCode', 'JobApply', 'ShortList', 'Payment', 'JobNotification', 'Swear', 'PostCode', 'CoverLetter', 'Skill', 'Location', 'Education', 'Course', 'AlertLocation', 'Alert', 'AlertJob', 'Designation', 'AutoAlert', 'AutoJob', 'Plan', 'Keyword', 'Feed','Mail','JobApplyCandidate','JobCredit');
        ...
        if ($this->data) {
            ...
            if (isset($msgString) && $msgString != '') {
                ...
                if ($this->Job->save($this->data)) {
					...
					$toEmail = base64_encode($jobDetail['User']['email_address']);
    ...
    public function admin_deletejobs($slug = NULL) {
        ...
        if ($slug != '') {
			...
			$this->AlertJob->deleteAll(array('AlertJob.job_id' => $id));
			$this->JobApply->deleteAll(array('JobApply.job_id' => $id));
			$this->JobNotification->deleteAll(array('JobNotification.job_id' => $id));
			$this->Mail->deleteAll(array('Mail.job_id' => $id));
			$this->JobApplyCandidate->deleteAll(array('JobApplyCandidate.job_id' => $id));
			$this->ShortList->deleteAll(array('ShortList.job_id' => $id));
			if(!empty($jobDetail['Job']['logo'])){
				if (file_exists(UPLOAD_JOB_LOGO_PATH . $jobDetail['Job']['logo'])) {
					@unlink(UPLOAD_JOB_LOGO_PATH . $jobDetail['Job']['logo']);
				}
	...
	public function cronEmail() {
		...
		file_put_contents("dinesh.txt",base64_decode($this->data['toEmail']));
		$this->Email->to = base64_decode($this->data['toEmail']);
		$this->Email->subject =  base64_decode($this->data['subjectToSend']);
		$this->Email->replyTo =  base64_decode($this->data['replyTo']);
		$this->Email->from =  base64_decode($this->data['from']);
		$this->Email->layout = $this->data['layout'];
		$this->set('messageToSend', base64_decode($this->data['messageToSend']));
		if(isset($this->data['pdfAttach'])){
			if(!empty($this->data['pdfAttach'])){
				$pdfAttach = json_decode(base64_decode($this->data['pdfAttach']),true);
				//$pdfAttach = base64_decode($this->data['pdfAttach']);
				$this->Email->attachments = $pdfAttach;
			}
		}
		$this->Email->template = 'email_template';
		$this->Email->sendAs = 'html';
	...
	public function testdinesh(){
		$site_title = $this->getSiteConstant('title');
        $mail_from = $this->getMailConstant('from');
		$toEmail = base64_encode("bhanu.surnilla@reymould.com");
		$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
		$from = base64_encode($site_title . "<" . $mail_from . ">");
		$layout = 'default';
		
		$username = "Test Job";
		$title = "Test";$category = "category";$location = "location";$skill = "skill";$designation = "designation";$minExp = "Test";$maxExp = "Test";$min_salary = "Test";$max_salary = "Test";
		$description = "Test";$company_name = "Test";$contact_number = "Test";$website = "Test";$address = "Test";
		$currentYear = date('Y', time());
		$sitelink = '<a style="color:#000; text-decoration: underline;" href="mailto:' . $mail_from . '">' . $mail_from . '</a>';
		...
		$messageToSend = base64_encode($messageToSend);
		$subjectToSend = base64_encode($subjectToSend);
		$showOldImages = $this->Certificate->find('list', array('conditions' => array('Certificate.user_id' => '968'),'fields'=>array('document')));
		$mailFileArray = array();
		if ($showOldImages) {
		...
		$pdfAttach =base64_encode(json_encode($mailFileArray));
		$cronUrl = base64_encode(BACKGROUND_PATH);
		$path = WWW_ROOT.'cronEmail.php';
		exec("php -f  $path  $toEmail $subjectToSend $replyTo $from $layout $messageToSend $cronUrl $pdfAttach >/dev/null 2>&1 &",$exeoutput);
		die;
    ...
    public function createJob($isCopy = null) {
        ...
        /*$locationlList = $this->Location->find('list', array('conditions' => array('Location.status' => '1'), 'fields' => array('Location.id', 'Location.name'), 'order' => 'Location.name asc'));
        $this->set('locationlList', $locationlList);*/
		$_SESSION['type'] = 'Gold';$_SESSION['amount'] = '180.00';
		if ($this->data) {
			...
			if (empty($this->data["Job"]["job_city"])) {
                $msgString .= __d('controller', 'Job location is required field.', true) . "<br>";
            } 
            ...
            if($this->data['Job']['jdoptions'] == "Doc" && empty($this->data["Job"]["description_document"]["name"])){
                $msgString .= __d('controller', 'Job description is required field.', true) . "<br>";
            }

            if($this->data['Job']['jdoptions'] == "Url" && empty($this->data["Job"]["description_url"])){
                $msgString .= __d('controller', 'Job description is required field.', true) . "<br>";
            }
            else if($this->data['Job']['jdoptions'] == "Url" && !empty($this->data["Job"]["description_url"])){
                if (filter_var($this->data["Job"]["description_url"], FILTER_VALIDATE_URL) == false) {
                    $msgString .= __d('controller', 'Invalid Job description URL.', true) . "<br>";
                }
            }
            
            if($this->data['Job']['jdoptions'] == "Text" && empty($this->data["Job"]["description"])){
                $msgString .= __d('controller', 'Job description is required field.', true) . "<br>";
            }
            else{
			...
			if (!empty($this->data["Job"]["description_document"]["name"])) {
                if ($this->data['Job']['description_document']['size'] > '10485760') {
                    $msgString .= __d('controller', 'Max document size upload is 10MB.', true) . "<br>";
                } 
            } 
			...
			else 
			{
				...
				if (!empty($this->data["Job"]["salary"])) {
					...
					//$this->request->data["Job"]["min_salary"] = $sallery[0];
					//$this->request->data["Job"]["max_salary"] = $sallery[1];
					$this->request->data["Job"]["min_salary"] = $this->data["Job"]["salary"];
					$this->request->data["Job"]["max_salary"] = $this->data["Job"]["salary"];
				}
                ...
                if ($this->data['Job']['jdoptions'] == "Doc" && !empty($this->data["Job"]["description_document"]["name"])) {
                    ...
                    $specialCharacters = array('#', '$', '%', '@', '+', '=', '\\', '/', '"', ' ', "'", ':', '~', '`', '!', '^', '*', '(', ')', '|', "'");
                    $toReplace = "-";
                    $this->request->data["Job"]["description_document"]['name'] = str_replace($specialCharacters, $toReplace, $this->data["Job"]["description_document"]['name']);
                    $this->request->data["Job"]["description_document"]['name'] = str_replace("&", "and", $this->data["Job"]["description_document"]['name']);
                    if ($this->data["Job"]["description_document"]['name']) {
                        $cvArray = $this->data["Job"]["description_document"];
                        $returnedUploadCVArray = $this->PImage->upload($cvArray, UPLOAD_JOB_DESCRIPTION_PATH);
                        $this->request->data["Job"]["description_document"] = $returnedUploadCVArray[0];
                        $this->request->data['Job']['description_url'] = null;
                        $this->request->data['Job']['description'] = "";
                    }
                }
                else if($this->data['Job']['jdoptions'] == "Url") 
				{
                    $this->request->data['Job']['description_document'] = null;
                    $this->request->data['Job']['description'] = "";
                }
                elseif($this->data['Job']['jdoptions'] == "Text")
                {
                    $this->request->data['Job']['description_document'] = null;
                    $this->request->data['Job']['description_url'] = null;
                }
				...
				$this->request->data['Job']['expire_time'] = strtotime('2099-01-01');
				$this->request->data['Job']['skill'] = implode(',', $this->data['Job']['skill']);
				...
				if ($_SESSION['amount'] > 0) {
                    if ($this->Job->save($this->data)) {
                       ...
                       // $this->Email->replyTo = $site_title . "<" . $mail_from . ">";
						...
						$pdfAttach ="";
						$cronUrl = base64_encode(BACKGROUND_PATH);
						//exec("F:/xampp/php/php.exe -f  $path  $toEmail $subjectToSend $replyTo $from $layout $messageToSend  2>&1 &",$exeoutput);
						exec("php -f  $path  $toEmail $subjectToSend $replyTo $from $layout $messageToSend $cronUrl $pdfAttach >/dev/null 2>&1 &",$exeoutput);
...
//                                pr($user);
                                ...
                                $this->request->data['AlertJob']['alert_id'] = $user['Alert']['id'];
                                $this->request->data["AlertJob"]['status'] = 1;
                                $this->request->data["AlertJob"]['view_status'] = 1;
                                if($this->AlertJob->save($this->data)){
                                    ...
                                    if ($user['User']['email_address']) {
                                        ...
                                        // $this->Email->template = 'email_template';
                                        // $this->Email->sendAs = 'html';
                                        // if(!$this->Email->send($messageToSend)){
                                        ...
                                        $toEmail = base64_encode($user['User']['email_address']);
                                        $replyTo = base64_encode($site_title . "<" . $mail_from . ">");
                                        $from = base64_encode($site_title . "<" . $mail_from . ">");
                                        $layout = 'default';
                                        $messageToSend = base64_encode($messageToSend);
                        ...
                        unset($_SESSION['promo_code']);
						if($verifiedUser){
							$jobCreditInfo = $this->JobCredit->findByJobId($jobId);
							if(empty($jobCreditInfo['JobCredit'])){
								$this->JobCredit->create();
								$this->request->data["JobCredit"]['job_id'] = $jobId;
								$this->request->data["JobCredit"]['user_id'] = $this->Session->read('user_id');
								$this->request->data['JobCredit']['credit'] = 20;
								$this->JobCredit->save($this->data);
								if(!empty($userDetails['User']['credit'])){
									$userCredit = $userDetails['User']['credit']+20;
								}
								else
								{
									$userCredit =20;
								...
								$this->User->updateAll(array('User.credit'=>$userCredit), array('User.id' => $userId));
							}
							$this->Session->write('success_msg', __d('controller', 'Your job posted successfully and 20 credits have been added to your account.', true));
						}
						else
						{
		...
		else 
		{
            ...
            if (isset($_SESSION['copy_data']) && $_SESSION['copy_data'] != '') {
                ...
                $this->request->data['Job']['salary'] = $this->data['Job']['min_salary'];
                $this->request->data['Job']['exp'] = $this->data['Job']['min_exp'] . '-' . $this->data['Job']['max_exp'];
				$this->request->data['Job']['skill'] = explode(',', $this->data['Job']['skill']);
			}
        }
    ...
    public function addFreeJob() {
        ...
        if ($this->Job->save($this->data)) {
           ...
           // $this->Email->subject = $subjectToSend;
			...
			$messageToSend = base64_encode($messageToSend);
			$subjectToSend = base64_encode($subjectToSend);
			$mailFileArray[] = array(UPLOAD_FULL_INVOICE_IMAGE_PATH . $jobInfo["Job"]["invoice_inumber"] . '.pdf');
			$pdfAttach =base64_encode(json_encode($mailFileArray));
			$path = WWW_ROOT.'cronEmail.php';
			$cronUrl = base64_encode(BACKGROUND_PATH);
			exec("php -f  $path  $toEmail $subjectToSend $replyTo $from $layout $messageToSend $cronUrl $pdfAttach >/dev/null 2>&1 &",$exeoutput);
    ...
    public function management() {
        ...
        $new_job_applicants = $this->JobApply->find('all', array(
            'conditions' => array($condition, 'JobApply.global_new_status' => 1),
            'fields' => array('JobApply.id' )
        ));

        if(!empty($new_job_applicants)){
            foreach($new_job_applicants as $new_applicant){
                $this->JobApply->updateAll(array('JobApply.global_new_status' => 0), array('JobApply.id' => $new_applicant['JobApply']['id']));
            }
        }
    ...
    public function edit($slug = null) {
        ...
        if ($this->data) {
            ...
            if($this->data['Job']['jdoptions'] == "Doc" && empty($this->data["Job"]["description_document"]["name"]) && empty($this->data["Job"]["old_description_document"])){
                $msgString .= __d('controller', 'Job description is required field.', true) . "<br>";
            }
            ...
            if (isset($msgString) && $msgString != '') {
                ...
                if($this->data['Job']['jdoptions'] == "Doc"){
                    if ($this->data["Job"]["description_document"]["name"]) {
                        $specialCharacters = array('#', '$', '%', '@', '+', '=', '\\', '/', '"', ' ', "'", ':', '~', '`', '!', '^', '*', '(', ')', '|', "'");
                        $toReplace = "-";
                        $this->request->data["Job"]["description_document"]['name'] = str_replace($specialCharacters, $toReplace, $this->data["Job"]["description_document"]['name']);
                        ...
                        //remove old desc doc
                        if(isset($this->data['Job']['old_description_document'])){
                            if (file_exists(UPLOAD_JOB_DESCRIPTION_PATH . $this->data['Job']['old_description_document'])) {
                                @unlink(UPLOAD_JOB_DESCRIPTION_PATH . $this->data['Job']['old_description_document']);
                            }
                        }
                    } else {
                        $this->request->data['Job']['description_document'] = $this->data['Job']['old_description_document'];
                    }
                    $this->request->data['Job']['description_url'] = null;
                    $this->request->data['Job']['description'] = "";
                }
                elseif($this->data['Job']['jdoptions'] == "Url"){
                    //remove old desc doc
                    if(isset($this->data['Job']['old_description_document'])){
                        if (file_exists(UPLOAD_JOB_DESCRIPTION_PATH . $this->data['Job']['old_description_document'])) {
                            @unlink(UPLOAD_JOB_DESCRIPTION_PATH . $this->data['Job']['old_description_document']);
		...
		else 
		{
            ...
            $this->request->data['Job']['old_description_document'] = $this->data['Job']['description_document'];
            $this->request->data['Job']['old_logo'] = $this->data['Job']['logo'];
            $this->request->data['Job']['old_title'] = $this->data['Job']['title'];
            $this->request->data['Job']['subcategory_id'] = explode(',', $this->data['Job']['subcategory_id']);
    ...
    public function sendMail($slug = null) {
        ...
        if ($this->data) {
            ...
            foreach ($allemails as $email) {
                if (trim($email) != '') {
					$candidateProfile = $this->JobApplyCandidate->find('first', array('conditions' => array('JobApplyCandidate.email' => $email, 'JobApplyCandidate.job_id' => $jobInfo['Job']['id'])));
                    if (!empty($candidateProfile)) {
						$candidateUserId = $candidateProfile['JobApplyCandidate']['user_id'];
                        $userInfo = $this->User->find('first', array('conditions' => array('User.id' => $candidateUserId)));
                       ...
                       // $this->Email->to = $email;
						...
						$maildata['Mail']['job_id'] = $jobInfo['Job']['id'];
						$maildata['Mail']['slug'] = $this->stringToSlugUnique($subject, 'Mail', 'slug');;
						$maildata['Mail']['subject'] = $subject;
						$maildata['Mail']['message'] = $message;
						$maildata['Mail']['status'] = '1';
						$maildata['Mail']['job_apply'] = '1';
						$this->Mail->save($maildata);
							
						
						// end mail insert code
...
//        }
	...
	public function importjob() {
        ...
        $tagline = $this->getSiteConstant('tagline');
        $title_for_pages = $site_title . " :: " . $tagline . " - ";
        $this->set('title_for_layout', $title_for_pages . __d('user', 'Import Jobs', true));
        ...
        if ($this->data) {
			...
			$rowNumber = 2;$totalInsertedRow = 0;$errorArray = array();$rowNumberError = array();$k = 0;
			for ($dd = 2; $dd <= $highestRow; $dd++) {
                ...
                $category = trim($row3);
                $jobId = trim($row4);
                $jobDescription = trim($row5);
                $companyName = trim($row6);
                $workT = trim($row7);
                $companyWebsite = trim($row8);
                $skills = trim($row9);
                $designation = trim($row10);
                $location = trim($row11);
                $experiance = trim($row12);
                $salary = trim($row13);
				...
				$resourceList[$k]['Serial Number'] = $sr_number;
                $resourceList[$k]['Job Title'] = $jobTitle;
                $resourceList[$k]['Category'] = $category;
                $resourceList[$k]['Job Id'] = $jobId;
                $resourceList[$k]['Job Description'] = $jobDescription;
                $resourceList[$k]['Company Name'] = $companyName;
                $resourceList[$k]['Work Type'] = $workT;
                $resourceList[$k]['Company Website'] = $companyWebsite;
                $resourceList[$k]['Skills'] = $skills;
                $resourceList[$k]['Designation'] = $designation;
                $resourceList[$k]['Location'] = $location;
                $resourceList[$k]['Experiance'] = $experiance;
				$resourceList[$k]['Annual Salary'] = $salary;
                

                $k++;
            }
			$error = 0;$suces = 0; $msgString = '';
			global $worktype;global $experienceArray;
            ...
            if ($resourceList) {
                ...
                foreach ($resourceList as $resource) {
                    $errorArray = array();
					if (empty($resource['Serial Number'])) {
                        $errorArray[] = 'Serial Number';
                        $errorMessage .= __d('controller', 'Serial Number is required field.', true) . "<br>";
                    }
					if (empty($resource['Job Title'])) {
                        $errorArray[] = 'Job Title';
                        $errorMessage .= __d('controller', 'Job Title is required field.', true) . "<br>";
                    }
					if (empty($resource['Category'])) {
                        $errorArray[] = 'Category';
                        $errorMessage .= __d('controller', 'Category is required field.', true) . "<br>";
                    }
					if (empty($resource['Job Description'])) {
                        $errorArray[] = 'Job Description';
                        $errorMessage .= __d('controller', 'Job Description is required field.', true) . "<br>";
                    }
					if (empty($resource['Company Name'])) {
                        $errorArray[] = 'Company Name';
                        $errorMessage .= __d('controller', 'Company Name is required field.', true) . "<br>";
                    }
					if (empty($resource['Work Type'])) {
                        $errorArray[] = 'Work Type';
                        $errorMessage .= __d('controller', 'Work Type is required field.', true) . "<br>";
                    }
					if (empty($resource['Skills'])) {
                        $errorArray[] = 'Skills';
                        $errorMessage .= __d('controller', 'Skills is required field.', true) . "<br>";
                    }
					if (empty($resource['Designation'])) {
                        $errorArray[] = 'Designation';
                        $errorMessage .= __d('controller', 'Designation is required field.', true) . "<br>";
                    }
					if (empty($resource['Location'])) {
                        $errorArray[] = 'Location';
                        $errorMessage .= __d('controller', 'Location is required field.', true) . "<br>";
                    }
					if (empty($resource['Experiance'])) {
                        $errorArray[] = 'Experiance';
                        $errorMessage .= __d('controller', 'Experiance is required field.', true) . "<br>";
                    }
					if (empty($resource['Annual Salary'])) {
                        //$errorArray[] = 'Annual Salary';
                      //  $errorMessage .= __d('controller', 'Annual Salary is required field.', true) . "<br>";
                    }
					
					if ($resource["Category"]) {
                        $category = $resource["Category"];
                        $categoryData = $this->Category->field('Category.id', array('Category.name' => $category));
                        if (!$categoryData) {
                            $errorArray[] = 'Category';
                            $errorMessage .= __d('controller', 'Please provide valid category name.', true) . "<br>";
                        }
                    }
					if ($resource["Experiance"]) {
                        $experience = $resource["Experiance"];
						
						if (!in_array($experience, $experienceArray)) {
							$errorArray[] = 'Experiance';
                            $errorMessage .= __d('controller', 'Please provide valid experience.', true) . "<br>";
                        }
                    }
					if ($resource["Work Type"]) {
                        $workT = $resource["Work Type"];
						
						if (!in_array($workT, $worktype)) {
							$errorArray[] = 'Work Type';
                            $errorMessage .= __d('controller', 'Please provide valid work type.', true) . "<br>";
                        }
                    }
					if ($resource["Skills"]) {
                        $skill = $resource["Skills"];
                        $skillCheck = $this->Skill->field('Skill.id', array('Skill.name' => $skill,'Skill.type' => 'Skill', 'Skill.status' => '1'));
                        if (!$skillCheck) {
                            $errorArray[] = 'Skills';
                            $errorMessage .= __d('controller', 'Please provide valid skill name.', true) . "<br>";
                        }
                    }
					if ($resource["Designation"]) {
                        echo $designation = $resource["Designation"];
                        $designationCheck = $this->Skill->field('Skill.id', array('Skill.name' => $designation,'Skill.type' => 'Designation', 'Skill.status' => '1'));
                        if (!$designationCheck) {
                            $errorArray[] = 'Designation';
                            $errorMessage .= __d('controller', 'Please provide valid designation name.', true) . "<br>";
                        }
                    }
                    ...
                    if (isset($errorMessage) && $errorMessage != '') {
                        ...
                        $error = 1;
                        $NewArray[] = array(
                            'Serial Number' => $resource['Serial Number'],
                            'Job Title' => $resource['Job Title'],
                            'Category' => $resource['Category'],
							'Job Description' => $resource['Job Description'],
							'Company Name' => $resource['Company Name'],
							'Work Type' => $resource['Work Type'],
							'Skills' => $resource['Skills'],
							'Designation' => $resource['Designation'],
							'Location' => $resource['Location'],
							'Experiance' => $resource['Experiance'],
							'Annual Salary' => $resource['Annual Salary'],
							'error' => $newArr,
				...
				if (empty($NewArray)) {
                    foreach ($resourceList as $resource) {
						$serialNumber = trim($resource['Serial Number']);
                        $jobTitle = trim($resource['Job Title']);
						$category = trim($resource['Category']);
                        $jobId = trim($resource['Job Id']);
                        $jobDescription = trim($resource['Job Description']);
                        $companyName = trim($resource['Company Name']);
                        $workT = trim($resource['Work Type']);
                        $companyWebsite = trim($resource['Company Website']);
						$skillsChoose = trim($resource['Skills']);
                        $designationChoose = trim($resource['Designation']);
                        $location = trim($resource['Location']);
                        $experiance = trim($resource['Experiance']);
                        $salary = trim($resource['Annual Salary']);
                        $suces = 1;
						$this->Job->create();
						$this->request->data = array();
						$salt = uniqid(mt_rand(), true);
						// find keyword
						$keyword = trim($jobTitle);
						$keywordId = $this->Keyword->field('id', array('Keyword.name' => $keyword, 'Keyword.type' => 'Job'));
						if (!$keywordId) {
							$this->request->data['Keyword']['name'] = $keyword;
							$this->request->data['Keyword']['slug'] = $this->stringToSlugUnique($keyword, 'Keyword');
						...
						//end keyword
						// find category
						$categoryId = $this->Category->field('Category.id', array('Category.name' => $category));
						//end category
						// find skills
						$skillId = $this->Skill->field('Skill.id', array('Skill.name' => $skillsChoose,'Skill.type' => 'Skill', 'Skill.status' => '1'));
						//end skills
						// find designation
						$designationId = $this->Skill->field('Skill.id', array('Skill.name' => $designationChoose,'Skill.type' => 'Designation', 'Skill.status' => '1'));
						//end designation
						$findWork = array_search($workT, $worktype, true);
						$findExperiance = array_search($experiance, $experienceArray, true);
						...
						if (isset($userDetail['User']['company_contact']) && !empty($userDetail['User']['company_contact'])) {
                           $contactNumber = $userDetail['User']['company_contact'];
                        } 
						
						$this->request->data['Job']['user_plan_id'] = $isAbleToJob['user_plan_id'];
						$this->request->data['Job']['job_id'] = 0;
						$this->request->data['Job']['jobID'] = $jobId;
                        $this->request->data['Job']['job_number'] = $serialNumber;
                        $this->request->data['Job']['user_id'] = $userId;
                        $this->request->data['Job']['admin_id'] = 0;
                        $this->request->data['Job']['type'] = $_SESSION['type'];
                        $slug = $this->stringToSlugUnique($jobTitle, 'Job');
						$this->request->data['Job']['slug'] = $slug;
						$this->request->data['Job']['title'] = $jobTitle;
                        $this->request->data['Job']['category_id'] = $categoryId;
                        ...
                        $this->request->data['Job']['job_city'] = $location;
						$this->request->data['Job']['price'] = 0;
                        $this->request->data['Job']['price_status'] = 1;
						$this->request->data['Job']['role'] = "";
                        $this->request->data['Job']['description'] = $jobDescription;
						$this->request->data['Job']['company_name'] = $companyName;
                        $this->request->data['Job']['work_type'] = $findWork;
						$this->request->data['Job']['contact_name'] = $contactName;
						$this->request->data['Job']['contact_number'] = $contactNumber;
						$this->request->data['Job']['vacancy'] = 0;
						$this->request->data['Job']['address'] = "";
						$this->request->data['Job']['state_id'] = 0;
						$this->request->data['Job']['city_id'] = 0;
						$this->request->data['Job']['postal_code'] = "";
						$this->request->data['Job']['url'] = $companyWebsite;
						$this->request->data['Job']['youtube_link'] = "";
						$this->request->data['Job']['lastdate'] = "";
						$this->request->data['Job']['slug'] = $slug;
						$this->request->data['Job']['status'] = 1;
						$this->request->data['Job']['job_status'] = 0;
						$this->request->data['Job']['created'] = date('Y-m-d H:i:s');
						$this->request->data['Job']['modified'] = date('Y-m-d H:i:s');
						$this->request->data['Job']['payment_status'] = 2;
						$this->request->data['Job']['amount_paid'] = "180.00";
						$this->request->data['Job']['logo'] = "";
						$this->request->data['Job']['expire_time'] = strtotime("2099-01-01");
						$this->request->data['Job']['dis_amount'] = "";
						$this->request->data['Job']['promo_code'] = "";
						...
						$this->request->data['Job']['view_count'] = 0;
						$this->request->data['Job']['job_logo_check'] = 0;
						$this->request->data['Job']['invoice_inumber'] = "";
						$this->request->data['Job']['payment_type'] = "";
						$this->request->data['Job']['weekly_email_sent'] = 0;
						$this->request->data['Job']['exp_year'] = 0;
						$this->request->data['Job']['exp_month'] = 0;
						if (!empty($findExperiance)) {
							$exp = explode('-', $findExperiance);
							$this->request->data["Job"]["min_exp"] = $exp[0];
							$this->request->data["Job"]["max_exp"] = $exp[1];
						}
						if (!empty($salary)) {
							$this->request->data["Job"]["min_salary"] = $salary;
							$this->request->data["Job"]["max_salary"] = $salary;
						}

						$this->request->data['Job']['confidential'] = "";
						$this->request->data['Job']['job_position'] = "";
						$this->request->data['Job']['eligibility'] = "";
						$this->request->data['Job']['job_salary'] = "";
						$this->request->data['Job']['job_experience_id'] = "";
						$this->request->data['Job']['job_email'] = "";
						$this->request->data['Job']['job_fax'] = "";
						$this->request->data['Job']['designation_ofperson'] = "";
						$this->request->data['Job']['brief_abtcomp'] = "";
						$this->request->data['Job']['lat'] = "";
						$this->request->data['Job']['long'] = "";
			...
			elseif ($suces == 1) {
                $this->Session->write('success_msg', __d('controller', 'Jobs details saved successfully.'));
                $this->redirect('/jobs/importjob');
                exit;
            } 
			else 
    ...
    public function old_listing($categorySlug = '') {
	...
	public function listing($categorySlug = '') {
        ...
        if(empty($userId)){
            $this->Session->write("returnUrl", $this->request->url);    
        }
        ...
        //$condition = array('Job.status' => 1, 'Job.payment_status' => 2, 'Job.expire_time >=' => time());
        $condition = array('Job.status' => 1, 'Job.expire_time >=' => time());
		$separator = array();$urlSeparator = array();$order = '';

        if (!empty($this->data)) {

            if(isset($this->data['alert_id']) && !empty($userId)){
                $this->AlertJob->updateAll(array("AlertJob.view_status" => "'0'"), array('AlertJob.user_id' => $userId, 'AlertJob.alert_id' => $this->data['alert_id']));
            }

            if (isset($this->data['Job']['searchRelevantJob']) && $this->data['Job']['searchRelevantJob'] == true) {
                if (!empty($userDetails['User']['skills'])) {
                    $skill_arr = explode(",", $userDetails['User']['skills']);
                    foreach ($skill_arr as $skillhave) {
                        $skillDetail = $this->Skill->find('first', array('conditions' => array('Skill.name'=>$skillhave)));
                        if ($skillDetail) {
                            $idshave = $skillDetail['Skill']['id'];
                            $condition_skill[] = "(FIND_IN_SET('" . $idshave . "',Job.skill))";
                        }
                ...
                $this->set('searchedRelevantJob', true);
            }
            ...
            if (isset($this->data['Job']['skill']) && !empty($this->data['Job']['skill'][0])) {
                foreach ($this->data['Job']['skill'] as $skillhave) {
                    $skillDetail = $this->Skill->find('first', array('conditions' => array('Skill.name'=>$skillhave)));
                    if ($skillDetail) {
                        $idshave = $skillDetail['Skill']['id'];
                        $condition_skill[] = "(FIND_IN_SET('" . $idshave . "',Job.skill))";
			...
			if (isset($this->data['Job']['company_name']) && $this->data['Job']['company_name'] != '') {
                $company_name = $this->data['Job']['company_name'];
            }
			if (isset($this->data['Job']['work_place_type']) && $this->data['Job']['work_place_type'] != '') {
                $work_place_type = $this->data['Job']['work_place_type'];
            }
			if (isset($this->data['Job']['job_city']) && $this->data['Job']['job_city'] != '') {
                $job_city = $this->data['Job']['job_city'];
            }
			if (isset($this->data['Job']['date_posted']) && $this->data['Job']['date_posted'] != '') {
                $date_posted = trim($this->data['Job']['date_posted']);
            }
            if (isset($this->data['Job']['work_type']) && !empty($this->data['Job']['work_type']) && count($this->data['Job']['work_type']) > 0) {
                if (is_array($this->data['Job']['work_type'])) {
                    $worktype = implode('-', $this->data['Job']['work_type']);
		...
		elseif (!empty($this->params)) {
			...
			if (isset($this->params['named']['company_name']) && $this->params['named']['company_name'] != '') {
                $company_name = trim($this->params['named']['company_name']);
            }
			if (isset($this->params['named']['work_place_type']) && $this->params['named']['work_place_type'] != '') {
                $work_place_type = trim($this->params['named']['work_place_type']);
            }
			if (isset($this->params['named']['date_posted']) && $this->params['named']['date_posted'] != '') {
                $date_posted = trim($this->params['named']['date_posted']);
            }
            if (isset($this->params['named']['skill']) && $this->params['named']['skill'] != '') {
                $skill = trim($this->params['named']['skill']);
                $skill = addslashes($skill);
		...
		if (isset($keyword) && $keyword != '') {
            ...
            $keyword_array = array_filter($keyword_array, function($value) {
                return !empty($value);
            });
            ...
            foreach($keyword_array as $keyword){
                ...
                $condition123[] = " (`Job`.`title` LIKE '%" . addslashes($keyword) . "%' OR `Job`.`description` LIKE '%" . addslashes($keyword) . "%' OR `Job`.`company_name` LIKE '%" . addslashes($keyword) . "%' ) ";
                
                $cbsd1 = "(Skill.name LIKE '%" . addslashes($keyword) . "%' and type='Designation')";
                $dDetail = $this->Skill->find('all', array('conditions' => $cbsd1));
                if ($dDetail) {
                    foreach($dDetail as $dD){
                        $idshave1 = $dD['Skill']['id'];
                        $condition123[] = "(FIND_IN_SET('" . $idshave1 . "',Job.designation))";
                    }
                    
                }
                $cbd1 = "(Skill.name LIKE '%" . addslashes($keyword) . "%' and type='Skill')";
                $skillDetail = $this->Skill->find('all', array('conditions' => $cbd1));
                if ($skillDetail) {
                    foreach($skillDetail as $sD){
                        $idshave = $sD['Skill']['id'];
                        $condition123[] = "(FIND_IN_SET('" . $idshave . "',Job.skill))";
                    }
                }
            }

            $condition[] = array('OR' => $condition123);
            $keyword = str_replace('\_', '_', $keyword);
		...
		$group = array();
		if(!empty($lat) && !empty($long) && !empty($location)){
			
			$latitude = $lat;
			$longitude = $long;
			//  $this->Job->virtualFields = array('distance' => "( 3959 * acos( cos( radians($latitude) ) * cos( radians( Job.lat ) ) * cos( radians( Job.long ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( Job.lat ) ) ) )");
			// $group[] = 'Job.id HAVING Job__distance <= 50';
			//$order = 'distance Asc';
            
            $location_condition[] = "( 3959 * acos( cos( radians($latitude) ) * cos( radians( Job.lat ) ) * cos( radians( Job.long ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( Job.lat ) ) )) <= 50";
            $condition[] = array('AND' => $location_condition);

            $urlSeparator[] = 'lat:' . $latitude;
            $separator[] = 'lat:' . $latitude;
            $urlSeparator[] = 'long:' . $longitude;
            $separator[] = 'long:' . $longitude;
            $urlSeparator[] = 'location:' . $location;
            $separator[] = 'location:' . $location;
		}
		...
		if (!empty($date_posted)) {
			if($date_posted=='2'){
				$condition[] = "Job.created BETWEEN NOW() -INTERVAL 1 DAY AND NOW()";
			}
			else if($date_posted=='3'){
				$condition[] = "Job.created > NOW() -INTERVAL 7 DAY";
			}
			else if($date_posted=='4'){
				$condition[] = "Job.created > NOW() -INTERVAL 1 MONTH";
			}
		}
		if (!empty($company_name)) {
            if(is_array($company_name)){
                foreach($company_name as $cn){
                    $condition_company_name[] = " (`Job`.`company_name` LIKE '%" . addslashes($cn). "%' ) ";
                }
                $condition[] = array('OR' => $condition_company_name);
            }else{
                $condition[] = " (`Job`.`company_name` LIKE '%" . addslashes($company_name). "%' ) ";
            }
		}

		if (!empty($work_place_type)) {
            foreach($work_place_type as $wpt){
                $condition_work_place_type[] = " (`Job`.`work_place_type` = '" . $wpt . "') ";
            }
            $condition[] = array('OR' => $condition_work_place_type);
		}

		if (!empty($job_city)) {
            foreach($job_city as $jc){
                $condition_job_city[] = " (`Job`.`job_city` LIKE '%" . addslashes($jc). "%' ) ";
            }
            $condition[] = array('OR' => $condition_job_city);
		}
		...
		$this->set('separator', $separator);
		$this->set('urlSeparator', $urlSeparator);
		$this->paginate['Job'] = array('conditions' => $condition, 'order' => $order,'group'=>$group,'limit'=>'25');
		$this->set('jobs', $this->paginate('Job'));
        
		$log = $this->Job->getDataSource()->getLog(false, false);
		debug($log);

        foreach ($condition as $key => $value) {
            if (is_array($value) && isset($value['OR'])) {
                foreach ($value['OR'] as $innerCondition) {
                    if (strpos($innerCondition, 'Job.skill') !== false || strpos($innerCondition, 'company_name') !== false || strpos($innerCondition, 'job_city') !== false) {
                        unset($condition[$key]);
                        break;
                    }
                }
        ...
        $jobs = $this->Job->find('all', array('conditions' => $condition, 'order' =>$order,'group'=>$group));
        ...
        foreach ($jobs as $item) {
            ...
            $job_skills = explode(',', $item['Job']['skill']);
            foreach ($job_skills as $job_skill) {
                if (!in_array($job_skill, $distinctSkillIds)) {
                    $distinctSkillIds[] = $job_skill;
                    $jskill = ClassRegistry::init('Skill')->field('name', array('Skill.id' => $job_skill));
                    $distinctSkillNames[$jskill] = $jskill;
                }
            }

            $companyName = $item['Job']['company_name'];
            if (!in_array($companyName, $distinctCompanyNames)) {
                $distinctCompanyNames[$companyName] = $companyName;
            }

            $job_city = trim($item['Job']['job_city']);
            if (!in_array($job_city, $distinctJobCities)) {
                $distinctJobCities[$job_city] = $job_city;
            }
        }
        unset($distinctSkillIds);
        
        $this->set('skillList', $distinctSkillNames);
        $this->set('distinctJobCities', $distinctJobCities);
        $this->set('companyoption_dd', $distinctCompanyNames);
	...
	public function jobsDetails() {
		...
		if(!empty($_POST['jobId']) && !empty($_POST['jobSlug'])){
			...
			$jobdetails = $this->Job->find('first', array('conditions' =>array('Job.id'=>$jobId,'Job.slug'=>$jobSlug)));
	...
	public function ajaxJobSave() {
        ...
        if(!empty($_POST['jobId']) && !empty($_POST['jobSlug'])){
			$jobId = $_POST['jobId'];
			$jobSlug = $_POST['jobSlug'];
			$jobData = $this->Job->find('first', array('conditions' =>array('Job.id'=>$jobId,'Job.slug'=>$jobSlug)));
			$checkStatus = $this->ShortList->find('first', array('conditions' => array('ShortList.user_id' => $user_id, 'ShortList.job_id' => $jobId)));
			if (!empty($checkStatus)) {
				echo json_encode(array('status' => 'error', 'message' =>"You already saved this job"));
				exit;
			...
			else
			{
				...
				$this->request->data['ShortList']['job_id'] = $jobId;
				if ($this->ShortList->save($this->data)) {
					echo json_encode(array('status' => 'success', 'message' =>"Job added in saved Jobs list"));
					exit;
	...
	public function ajaxJobUnSave() {
        ...
        if(!empty($_POST['jobId']) && !empty($_POST['jobSlug'])){
			$jobId = $_POST['jobId'];
			$jobSlug = $_POST['jobSlug'];
			$this->ShortList->deleteAll(array('ShortList.job_id' => $jobId,'ShortList.user_id' => $user_id));
			echo json_encode(array('status' => 'success', 'message' =>"Job removed in saved Jobs list"));
			exit;
		}
		echo json_encode(array('status' => 'error', 'message' =>"Invalid Details"));
		exit;
	...
	public function ajaxApplyJob() {
		...
		if(empty($jobInfo)){
			$msgString .= __d('controller', 'Job is not valid now.', true) . "<br>";
		}
		if(empty($this->request->data['User']['email_address'])){
			 $msgString .= __d('controller', 'Email is required field.', true) . "<br>";
		}
		if(empty($this->request->data['User']['contact'])){
			 $msgString .= __d('controller', 'Phone number is required field.', true) . "<br>";
		}
		if(!isset($this->request->data['chooseCv'])){
			 $msgString .= __d('controller', 'Resume is required field.', true) . "<br>";
		}
		if(isset($this->request->data['chooseCv'])){
			if(empty($this->request->data['chooseCv'])){
				$msgString .= __d('controller', 'Resume is required field.', true) . "<br>";
			}
		}
		$isAbleToJob = $this->Plan->checkPlanFeature($userId, 4);
		if($isAbleToJob['status'] == 0) {
			$msgString .= $isAbleToJob['message'] . "<br>";
		}
		
		$checkStatus = $this->JobApply->find('first', array('conditions' => array('JobApply.user_id' => $userId, 'JobApply.job_id' => $job_id)));
        if (!empty($checkStatus)) {
            $msgString .= "- You already applied for this job <br>";
        }
		if(!empty($jobInfo)){
			$recruiterInfo = $this->User->findByid($jobInfo['Job']['user_id']);
			if(empty($recruiterInfo)){
				$msgString .= "- Job owner not valid now. <br>";
			}
		}
		if (isset($msgString) && $msgString != '') {
			echo json_encode(array('status' => 'error', 'message' =>$msgString));
			exit;
		} 
		else 
		{
            //check if already applied for job with similar jobId and company
            $already_applied_jobs = $this->JobApply->find('all', array(
                'conditions' => array('JobApply.user_id' => $userId),
                'fields' => array('JobApply.id', 'Job.id', 'Job.jobID', 'Job.company_name')
            ));
            if(!empty($already_applied_jobs)){
                foreach($already_applied_jobs as $applied_job){

                    $appliedjobID = strtolower($applied_job['Job']['jobID']);
                    $appliedCompanyName = strtolower($applied_job['Job']['company_name']);
                    $jobInfojobID = strtolower($jobInfo['Job']['jobID']);
                    $jobInfoCompanyName = strtolower($jobInfo['Job']['company_name']);
                    
                    if((!empty($appliedjobID) && $appliedjobID == $jobInfojobID) && ($appliedCompanyName == $jobInfoCompanyName) && !isset($this->request->data['forceApply'])){
                        echo json_encode(array('status' => 'pending', 'message' =>"You have applied to a similar job at <b>".$jobInfo['Job']['company_name']."</b> posted by a different employee at the same company. Do you still want to apply?. <br>"));
			            exit;
                    }
                }
            }
			...
			$this->request->data['JobApply']['global_new_status'] = 1;
			$this->request->data['JobApply']['status'] = 1;
			$this->request->data['JobApply']['apply_status'] = 'active';
			$this->request->data['JobApply']['user_id'] = $userId;
			...
			if ($this->JobApply->save($this->data)) {
				$recruiterEmail = $this->request->data['User']['email_address'];
				$this->request->data['JobApplyCandidate']['job_id'] = $this->request->data['Job']['id'];
				$this->request->data['JobApplyCandidate']['user_id'] = $userId;
				$this->request->data['JobApplyCandidate']['email'] = $this->request->data['User']['email_address'];
				$this->request->data['JobApplyCandidate']['phone_number'] = $this->request->data['User']['contact'];
				$source = UPLOAD_CERTIFICATE_PATH . $this->request->data['chooseCv'];
				$destination = UPLOAD_APPLY_JOB_CERTIFICATE_PATH . $this->request->data['chooseCv'];
				copy( $source, $destination );
				$this->request->data['JobApplyCandidate']['profile_resume'] = $this->request->data['chooseCv'];
				$this->JobApplyCandidate->save($this->data);
				...
				$mailFileArray = array();
				$mailFileArray[$this->request->data['chooseCv']] = $destination;
				...
				$link = HTTP_PATH . "/jobs/candidateDetails/" . $jobInfo['Job']['slug'] . '/' . $userInfo['User']['slug'];
				$emData = $this->Emailtemplate->getSubjectLang();
                $subjectField = $emData['subject'];
                $templateField = $emData['template'];
				$currentYear = date('Y', time());
                ...
                $fromRepArray = array($recruiterName, $jobTitle, $userName, $recruiterEmail, $currentYear, HTTP_PATH, $site_title, $sitelink, $site_url, $link);
                $messageToSend = str_replace($toRepArray, $fromRepArray, $emailtemplateMessage['Emailtemplate'][$templateField]);
                $toEmail = base64_encode($recruiterEmail);
				$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
				$from = base64_encode($site_title . "<" . $mail_from . ">");
				$layout = 'default';
				$messageToSend = base64_encode($messageToSend);
				$subjectToSend = base64_encode($emailtemplateMessage['Emailtemplate'][$subjectField]);
				$path = WWW_ROOT.'cronEmail.php';
				$pdfAttach ="";
				...
				echo json_encode(array('status' => 'success','jobId' => $job_id,'message' =>__d('controller', 'Your job application is successfully posted. We will contact you soon.', true)));
            }
			else
			{
				echo json_encode(array('status' => 'error', 'message' =>"There is some tecnical error.please try again later."));
				exit;
			}
	...
	public function employeeVerficationLater(){
		$this->autoRender = false;
        $this->layout = '';
		$this->Session->write('employeeVerficationLater', true);
		exit;
	}
	
	public function employeeVerficationLaterFirst(){
		$this->autoRender = false;
        $this->layout = '';
		$this->Session->write('employeeVerficationLaterFirst', true);
		//echo $this->Session->read('employeeVerficationLaterFirst')."dd";
		echo "true";
		exit;
	}
	
	
	public function employeeEmailVerfication(){
		$this->autoRender = false;
        $this->layout = '';
		$mail_from = $this->getMailConstant('from');
        $site_title = $this->getSiteConstant('title');
        $userId = $this->Session->read("user_id");
		if ($this->data) {
			if(!empty($this->request->data['User']['email_address']) && !empty($this->request->data['User']['email_otp'])){
				$email_otp = $this->request->data['User']['email_otp'];
				$userOtpCheck = $this->User->find('first', array('conditions' => array('User.email_otp' =>$email_otp, 'User.id' => $userId)));
				if(!empty($userOtpCheck)) {
					$this->User->id = $userId;
					$currentDate = date('Y-m-d');
					unset($this->request->data['User']['email_address']);
					unset($this->request->data['User']['verification_document']);
					if(!empty($userOtpCheck["User"]["verification_document"])){
						$verifiedDocument = $userOtpCheck["User"]["verification_document"];
						@unlink(UPLOAD_EMPLOYEE_VERIFICATION_PATH . $verifiedDocument);
					}
					$this->request->data['User']['verification_document'] = "";
					$this->request->data['User']['email_otp'] = "";
					$this->request->data['User']['emp_verification_status'] = "Verified";
					$this->request->data['User']['verification_source'] = "email";
					$this->request->data['User']['verification_date'] = $currentDate;
					$this->User->save($this->data);
					echo json_encode(array('status' => 'success', 'message' =>"Your employee verification has been successfully verified.",'type'=>'otp'));
					exit;
				}
				else
				{
					echo json_encode(array('status' => 'error', 'message' =>"OTP is incorrect.",'type'=>'otp'));
					exit;
				}
			}
			else if(!empty($this->request->data['User']['email_address'])){
				$email = $this->request->data['User']['email_address'];
				$emailDomainexcept = array("gmail.com","yahoo.com","yahoo.co.in","outlook.com");
				$emailExplode = explode('@', $email);
				$emailDomain = strtolower($emailExplode[1]);
				if(in_array($emailDomain,$emailDomainexcept)){
					echo json_encode(array('status' => 'error', 'message' =>"official emails are allowed.",'type'=>'email'));
					exit;
				}
				
				$referral_code = mt_rand(100000, 999999);
				$this->User->updateAll(array('User.email_otp' =>$referral_code), array('User.id' => $userId));
				$userCheck = $this->User->find("first", array("conditions" => "User.id = '" . $userId . "'"));
				$username = $userCheck["User"]["first_name"];
				 $currentYear = date('Y', time());
                    $emailtemplateMessage = $this->Emailtemplate->find("first", array("conditions" => "Emailtemplate.static_email_heading='employee_verification_email_otp'"));
                    $sitelink = '<a style="color:#000; text-decoration: underline;" href="mailto:' . $mail_from . '">' . $mail_from . '</a>';
                    ...
                    $toSubArray = array('[!username!]', '[!HTTP_PATH!]', '[!SITE_TITLE!]', '[!emailotp!]','[!DATE!]');
                    $fromSubArray = array($username, HTTP_PATH, $site_title, $referral_code,$currentYear);
                    $subjectToSend = str_replace($toSubArray, $fromSubArray, $emailtemplateMessage['Emailtemplate'][$subjectField]);
                    $toRepArray = array('[!username!]', '[!HTTP_PATH!]', '[!SITE_TITLE!]', '[!emailotp!]','[!DATE!]');
                    $fromRepArray = array($username, HTTP_PATH, $site_title, $referral_code,$currentYear);
                    $messageToSend = str_replace($toRepArray, $fromRepArray, $emailtemplateMessage['Emailtemplate'][$templateField]);
                    $this->Email->layout = 'default';
                    $this->set('messageToSend', $messageToSend);
					$toEmail = base64_encode($email);
					...
					$cronUrl = base64_encode(BACKGROUND_PATH);
					exec("php -f  $path  $toEmail $subjectToSend $replyTo $from $layout $messageToSend $cronUrl $pdfAttach >/dev/null 2>&1 &",$exeoutput);
					echo json_encode(array('status' => 'success', 'message' =>"OTP sent to on email address.",'type'=>'email'));
					exit;
			}
			else if(!empty($this->request->data['User']['email_otp'])){
				$email_otp = $this->request->data['User']['email_otp'];
				$userOtpCheck = $this->User->find('first', array('conditions' => array('User.email_otp' =>$email_otp, 'User.id' => $userId)));
				if(!empty($userOtpCheck)) {
					$this->User->id = $userId;
				...
				echo json_encode(array('status' => 'error', 'message' =>"Email address is required.",'type'=>'email'));
				exit;
			}
			else if(empty($this->request->data['User']['email_otp'])){
				echo json_encode(array('status' => 'error', 'message' =>"OTP is required.",'type'=>'email'));
				exit;
			}
			echo json_encode(array('status' => 'error', 'message' =>"email is required.",'type'=>'otp'));
			exit;
		}
	}
	
	public function employeeDocumentVerfication(){
		$this->autoRender = false;
        $this->layout = '';
		$mail_from = $this->getMailConstant('from');
        $site_title = $this->getSiteConstant('title');
        ...
        global $extentions_doc;
		$msgString="";
		if ($this->data['User']['verification_document']['name'] != '') {
			list($width, $height, $type, $attr) = getimagesize($this->data['User']['verification_document']['tmp_name']);
            $getextention = $this->PImage->getExtension($this->data['User']['verification_document']['name']);
            $extention = strtolower($getextention);
			...
			else 
			{
                if (in_array($extention, $extentions)) {
                    if ($this->data['User']['verification_document']['size'] > '4194304') {
            ...
            $msgString .= "- Document required. <br>";
        }
		...
		else 
		{
			...
			$this->User->id = $userId;
			$userCheck = $this->User->find("first", array("conditions" => "User.id = '" . $userId . "'"));
			if(!empty($userCheck["User"]["verification_document"])){
				$verifiedDocument = $userCheck["User"]["verification_document"];
				@unlink(UPLOAD_EMPLOYEE_VERIFICATION_PATH . $verifiedDocument);
			}
            if (in_array($extention, $extentions)) {
                $imageArray = $this->data['User']['verification_document'];
                $imageArray['name'] = $this->sanitizeFilename($this->data['User']['verification_document']['name']);
                $returnedUploadImageArray = $this->PImage->upload($imageArray, UPLOAD_EMPLOYEE_VERIFICATION_PATH, "jpg,jpeg,png,gif");
                if (isset($returnedUploadImageArray[1]) && !empty($returnedUploadImageArray[1])) {
				...
				else 
				{
                    ...
                    chmod(UPLOAD_EMPLOYEE_VERIFICATION_PATH .$returnedUploadImageArray[0], 0755);
					$currentDate = date('Y-m-d');
					unset($this->request->data['User']['email_address']);
					$this->request->data['User']['email_otp'] = "";
					$this->request->data['User']['emp_verification_status'] = "Approval Pending";
					$this->request->data['User']['verification_source'] = "document";
					$this->request->data['User']['verification_date'] = $currentDate;
					$this->request->data['User']['verification_document'] = $image;
                    $this->User->save($this->data);
                }
            } 
			elseif (in_array($extention, $extentions_doc)) {
                $specialCharacters = array('#', '$', '%', '@', '+', '=', '\\', '/', '"', ' ', "'", ':', '~', '`', '!', '^', '*', '(', ')', '|', "'");
                $toReplace = "-";
                $this->request->data['User']['verification_document']['name'] = str_replace($specialCharacters, $toReplace, $this->data['User']['verification_document']['name']);
                $this->request->data['User']['verification_document']['name'] = str_replace("&", "and", $this->data['User']['verification_document']['name']);
				
                if ($this->data['User']['verification_document']['name']) {
					
					$cvArray = $this->data['User']['verification_document'];
                    $returnedUploadCVArray = $this->PImage->upload($cvArray, UPLOAD_EMPLOYEE_VERIFICATION_PATH);
					
					if (isset($returnedUploadCVArray[1]) && !empty($returnedUploadCVArray[1])) {
						$msgString .= "- Image file not valid <br>";
					} 
					else 
					{
						$image = $returnedUploadCVArray[0];
						chmod(UPLOAD_EMPLOYEE_VERIFICATION_PATH .$returnedUploadCVArray[0], 0755);
						$this->request->data['User']['verification_document'] = $image;
						$currentDate = date('Y-m-d');
						unset($this->request->data['User']['email_address']);
						$this->request->data['User']['email_otp'] = "";
		...
		{
             echo json_encode(array('status' => 'success', 'message' => 'Your Document is submitted for Verification'));
...
//        exit;
        ...
        $job_id = $jobdetails['Job']['id'];
        $time = time();
        $jobcond = array("Job.status = 1 AND Job.id != $job_id AND Job.category_id = $cat_id AND Job.expire_time >= $time AND Job.user_id != $userId");
        $relevantJobList = $this->Job->find('all', array('conditions' => $jobcond, 'order' => 'Job.created Desc', 'limit' => '6'));
        $this->set('relevantJobList', $relevantJobList);
//        echo '<pre>';
...
//                  $this->Email->to = $recruiterEmail;
                   ...
                   // $this->Email->from = $site_title . "<" . $mail_from . ">";
                    ...
                    $this->Email->sendAs = 'html';
                    $this->Email->send();*/
					$toEmail = base64_encode($adminEmail);
					$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
					$from = base64_encode($site_title . "<" . $mail_from . ">");
					$layout = 'default';
					$messageToSend = base64_encode($messageToSend);
...
//                    $this->redirect(array('controller' => 'jobs', 'action' => 'listing'));
    ...
    public function candidateDetails($job_slug = null, $can_slug = null) {
       ...
       // $this->recruiterAccess();
		...
		$candidateProfile = $this->JobApplyCandidate->find('first', array('conditions' => array('JobApplyCandidate.user_id' => $userdetails['User']['id'], 'JobApplyCandidate.job_id' => $jobDetails['Job']['id'])));
    ...
    public function saveJobData() {
        ...
        if ($this->Job->save($this->data)) {
           ...
           // $this->Email->replyTo = $site_title . "<" . $mail_from . ">";
            ...
            $this->Email->sendAs = 'html';
            $this->Email->send();*/
			$toEmail = base64_encode($adminInfo['Admin']['acnt_email_add']);
			$replyTo = base64_encode($site_title . "<" . $mail_from . ">");
			$from = base64_encode($site_title . "<" . $mail_from . ">");
			$layout = 'default';
			$messageToSend = base64_encode($messageToSend);
