        ...
        if (!empty($jobId)) {
            ...
            if (!empty($jobDetail)) {
                $condition = array(
                    ...
                    "User.id > 0",
                    "Alert.skill IN (" . $jobDetail['Job']['skill'] .")",
                    'Alert.status' => 1,
                    "(Alert.designation IS NULL OR Alert.designation = 0 OR Alert.designation = ". $jobDetail['Job']['designation'].")"
                );
//                echo '<pre>';
//                print_r($condition);
                $users = ClassRegistry::init('Alert')->find('all', array('conditions' => $condition, 'fields' => array('DISTINCT(User.id), User.email_address, User.first_name, User.last_name, Alert.id')));
//                print_r($users);exit;
            }
        }
        return $users;
