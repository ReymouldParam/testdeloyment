...
//    );
    ...
    public function getcurrentplan($userId=null){
        $date = date('Y-m-d');
        $userPlan = Classregistry::init('UserPlan')->find('first', array('conditions'=>array('UserPlan.user_id'=>$userId, 'UserPlan.is_expire'=>0, 'UserPlan.start_date <='=>$date, 'UserPlan.end_date >='=>$date)));
        return $userPlan;
    }
