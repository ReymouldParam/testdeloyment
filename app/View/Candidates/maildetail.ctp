            ...
            <div class="my_acc">
                ...
                <div class="col-sm-9 col-lg-9 col-xs-12">
                    <div class="my-profile-boxes">
                        <div class="my-profile-boxes-top my-mail-boxes"><h2><i><?php echo $this->Html->image('front/home/mail-history-icon2.png', array('alt' => '')); ?></i><span><?php echo __d('user', 'Mail Detail', true); ?></span></h2>
                            ...
                            <div class="listing_page">
								<div class="jjj" id="job2" >
                                    <div class="search_full">
										...
										if($userdetail['User']['id']==$mailDetail['Mail']['to_id']){?>
						...
						<tbody>
							<tr>
								...
								<?php
									...
									if($userdetail['User']['id']==$mailDetail['Mail']['from_id']){
										$cslug = $jobInfo['Category']['slug'];
										$jobslug = $jobInfo['Job']['slug'];
										$applyCandidateInformation = classregistry::init('JobApplyCandidate')->find('first', array('conditions' => array('JobApplyCandidate.user_id' => $mailDetail['Mail']['to_id'], 'JobApplyCandidate.job_id' => $jobInfo['Job']['id'])));
										
										if(!empty($applyCandidateInformation)){
											$candidateEmail = $applyCandidateInformation['JobApplyCandidate']['email'];
										}
										else
										{
											$candidateEmail = $mailDetail['Sender']['email_address'];
										}
										?>
										<table width="100%">
											<tbody>
												...
												<tr>
													<td style="color:#434343; font-size:13px; line-height:18px;">
														...
														<p style="color:#434343; margin:10px 0 0;"><strong style="width:150px;"><?php echo __d('user', 'Email Address', true); ?>:</strong> <?php echo $candidateEmail; ?></p>
														<p style="color:#434343; margin:10px 0 0;"><a  href="<?php echo HTTP_PATH . '/jobs/candidateDetails/' .$mailDetail['Job']['slug'].'/'.$mailDetail['Sender']['slug'] ?>" style="color:#0d4f87;  "><?php echo __d('user', 'Click here', true); ?></a> <?php echo __d('user', 'to jobseeker details', true); ?>.</p>
														<?php
														if ($files) {
...
<?php echo $this->Form->create('null', array('url' => array('controller' => 'candidates', 'action' => 'sendmailemployer', $mailDetail['Mail']['slug']), 'enctype' => 'multipart/form-data', "method" => "Post", "id" => 'sendmail')); ?>
...
<?php echo __d('user', 'Select multiple file with Ctrl press,', true).' '.__d('user', 'Supported File Types', true); ?>: gif, jpg, jpeg, png, pdf, doc, docx (Max 5 images and Max. <?php echo $max_size; ?>MB).
                ...
                <div class="form-group">
                    ...
                    <div class="form-group-input">
                        <?php 
						echo $this->Form->hidden('Candidate.id', array('value' => $mailDetail['Company']['id'])); 
						
						?>
<?php echo $this->Form->hidden('Candidate.slug', array('value' => $mailDetail['Company']['slug'])); ?>
                        <?php echo $this->Form->hidden('Candidate.type', array('value' => 'candidates')); ?>
