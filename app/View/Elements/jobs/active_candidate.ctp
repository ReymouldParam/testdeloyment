...
<?php if ($candidates) { ?>
    ...
    <div class="table_over_auto_div">
        <div class="detl_content">
            ...
            <?php foreach ($candidates as $candidate) {
					$applyCandidateInformation = classregistry::init('JobApplyCandidate')->find('first', array('conditions' => array('JobApplyCandidate.user_id' => $candidate['User']['id'], 'JobApplyCandidate.job_id' => $candidate['JobApply']['job_id'])));

				?>
                <ul class="detl_list">
                    <li class="new_tested"><input type="checkbox" id="listemail" onclick="javascript:isAllSelect(this.form);" name="chkRecordId" value="<?php echo $applyCandidateInformation['JobApplyCandidate']['email']; ?>" /></li>
                    <li>
                        <div class="candi_dtl">
