...
<style>
    ...
    <div class="my-profile-boxes info_dv_esdit_pre" id="editProfileTab" style="display:none;">
        <div class="my-profile-boxes-top my-education-boxes"><h2><i><?php echo $this->Html->image('front/home/edit-icon2.png', array('alt' => '')); ?></i><span><?php echo __d('user', 'Edit Profile Information', true); ?></span></h2> <a href="javascript:;" class="closeProfileLNK"><i class="fa fa-close"></i>Close</a></div>
		<div class="information_cntn" style="position:inherit !important;">
            ...
            <div class="form_list_education">
                ...
                <div class="form_input_education">
                    <div class="user_img_box">
                        ...
                        ?>
                        <input type="hidden" id="images" name="data[User][old_image]" value="<?= $UseroldImage['User']['profile_image'] ?>" >
                        <div class="showchange showmede2 delete_icon" id="photo22">
                            <a class="edit_profilepicture" href="<?php echo HTTP_PATH; ?>/candidates/deleteImage/<?php echo $UseroldImage['User']['slug']; ?>" onClick="return confirm('<?php echo __d('user', 'Are you sure you want to Delete ?', true); ?>');">
                                ...
                                <?php
                                echo $this->Html->image(DISPLAY_THUMB_PROFILE_IMAGE_PATH . $UseroldImage['User']['profile_image'], array('escape' => false,'rel'=>'nofollow','id'=>'profileOutput'), array());
                            } else {
                                echo $this->Html->image('front/no_image_user.png' ,array('class'=>'image_css','id' => 'profileOutput'));
                            }
                        ?>
                    </div>    
                </div> 
            ...
            <div class="form_list_education">
                <label class="lable-acc"><?php echo __d('user', 'New Image', true);?></label>
                <div class="form_input_education form_upload_file">
                    <span class="choose-file-your">Choose File</span>
                    <?php echo $this->Form->file('User.profile_image', array('class' => 'form-control required', 'onchange'=> 'loadFile(event)','accept' => 'image/*')); ?>
                </div>
                <div class="abccc pstrength-minchar"><?php echo __d('user', 'Supported File Types', true);?>: gif, jpg, jpeg, png (Max. 2MB). <?php echo __d('user', 'Min file size', true);?> 250 X 250 <?php echo __d('user', 'pixels', true);?>.</div>
            </div>
			...
			<div class="form_list_education">
			   <label class="lable-acc"><?php echo __d('user', 'Contact Number', true); ?> <span class="star_red">*</span></label>
				<div class="form_input_education"><?php echo $this->Form->text('User.contact', array('maxlength' => '16', 'minlength' => '8', 'class' => "form-control required contact", 'placeholder' => __d('user', 'Contact Number', true))) ?>
			...
			<div class="form_list_education">
				...
				<div id="cust_skl" class="form_input_education rel_Skills">
                    ...
                    <?php echo $this->Form->select('User.skills', $skillList, array('multiple' => true, 'data-placeholder' => __d('user', 'Choose Skills', true), 'class' => "form-control select2 required",'data-type' => 'Skill')); ?>
...
<?php echo $this->Form->textarea('User.company_about', array('class' => "form-control", 'placeholder' => __d('user', 'About Your Self', true))) ?>
			...
			<div class="form_list_education">
				...
				<div id="cust_skl" class="form_input_education rel_Skills" >
<?php echo $this->Form->select('User.interest_categories', $categories, array('multiple' => true, 'data-placeholder' => __d('user', 'Select Interest Categories', true), 'class' => "form-control select2",'data-type' => 'Category')); ?>
					<label id="skillss1"></label>
				</div>
			</div>
			<div class="form_list_education">
			   ...
			   <div class="form_input_education">
					<div id="mulitplefileuploader_cv"><?php echo __d('user', 'Choose File', true); ?> </div>
							...
							foreach ($this->data['CoverLetter'] as $letter) {
								...
								<div class="media_box">
									...
									if (isset($letter['CoverLetter']['id']) && $letter['CoverLetter']['id'] != '') {
										echo '<span class="cover_ttt">' . $this->Form->text('CoverLetter.' . $coverCounter . '.title', array('maxlength' => '30', 'class' => "form-control required", 'placeholder' => __d('user', 'Cover Letter Title', true), 'value' => $letter['CoverLetter']['title'],'required'=>true)) . '</span>';

										echo $this->Form->hidden('CoverLetter.' . $coverCounter . '.id', array('value' => $letter['CoverLetter']['id']));

										echo '<span class="cover_desss">' . $this->Form->textarea('CoverLetter.' . $coverCounter . '.description', array('class' => "form-control required", 'placeholder' => __d('user', 'Cover Letter', true), 'value' => $letter['CoverLetter']['description'],'required'=>true)) . '</span>';
										$coverCounter++;
									}
									if (isset($letter['CoverLetter']['id']) && $letter['CoverLetter']['id'] != '') {
										?>
...
<script>
    ...
    <?php if(!empty($experienceDetails && $this->request->data['User']['total_exp'] > 0)) { ?>

        $('#UserTotalExp').on('change', function() {
            var selectedexp = $(this).val();
            if(selectedexp && selectedexp == 0 ){
                if (confirm('If Fresher is selected - the experience entered will be deleted. Are you sure?')) {
                    $(this).val(0);
                } else {
                    $(this).val('<?= $this->request->data['User']['total_exp'] ?>');
                }
            }
        });
    <?php } ?>
...
<script>
    ...
    var loadFile = function(event) {
        ...
        if (input.files && input.files[0]) {
            if (imageValidation(input.files[0])) {
    ...
    function imageValidation(file) {
        ...
        if (!filetype.includes(ext)) {
            ...
            var filesize = file.size; // check uploaded file size
            var max_size = <?php echo $max_size ?> * 1048576; // Max file size in bytes
