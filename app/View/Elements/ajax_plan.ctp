                                        ...
                                        foreach ($plans as $plan) {
                                            ...
                                            <div class="pl_main">
                                                ...
                                                if ($cplanId) {
                                                    ?>
                                                    <div class="plied_img">
                                                        <input class="plcheckbox" name="data[User][rememberme<?php echo $plan['Plan']['id']; ?>]" id="check<?php echo $plan['Plan']['id']; ?>" value="<?php echo $plan['Plan']['id']; ?>" type="checkbox"> <?php echo __d('user', 'Apply Immediately', true); ?>
...
//                                                    print_r($featureIds);
								...
								<?php echo $this->Form->hidden('Plan.job_id', array('value' =>$jobId)); ?>
								<?php echo $this->Form->hidden('Plan.job_slug', array('value' =>$jobSlug)); ?>
								<?php echo $this->Form->hidden('Plan.from_page', array('value' =>$fromPage)); ?>
...
<div id="paymentLoaderID" style="display:none;position:absolute;"><?php echo $this->Html->image("loader_large_blue.gif"); ?></div>
...
<script>
    ...
    function setplanid(pid, pname, sdate, edate, amount) {
        ...
        if (amount <= 0) {
			...
			var jobId = $("#PlanJobId").val();
			var jobSlug = $("#PlanJobSlug").val();
			var PlanFromPage = $("#PlanFromPage").val();
            //$("#purchaseplan").submit();
			$.ajax({
				type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
				url: "<?php echo HTTP_PATH . "/plans/ajaxPlanSubmit" ?>",
				data        :  $('#purchaseplan').serializeArray(), // our data object
				dataType    : 'json', // what type of data do we expect back from the server
				beforeSend: function () {
					$('#upgradePlanModal').css({'z-index':'0'});
					$('#paymentLoaderID').show();
				},
				complete: function () {
					$('#paymentLoaderID').hide();
